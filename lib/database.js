...
class DB {
  ...
  // ── Raw SQL 숏컷 (CRON 등에서 직접 SQL 실행용) ──────────────────────────
  /**
   * SELECT 다건 조회
   * @param {string} sql
   * @param {any[]} [params=[]]
   ...
   */
  all(sql, params = []) {
    return this.#conn.raw.prepare(sql).all(...params);
  }
   ...
   */
  get(sql, params = []) {
    return this.#conn.raw.prepare(sql).get(...params) || null;
  }
   ...
   */
  run(sql, params = []) {
    return this.#conn.raw.prepare(sql).run(...params);
  }
  ...
  #resolveRepoClass(name) {
    switch (name) {
      ...
      case 'severance_dispatch':        return require('./repositories/severanceDispatchRepository').SeveranceDispatchRepository;
      ...
      case 'payroll_items':             return require('./repositories/payrollItemsRepository').PayrollItemsRepository;
      case 'kakao_send_logs':           return require('./repositories/kakaoSendLogsRepository').KakaoSendLogsRepository;
      case 'attendance_ledger':         return require('./repositories/attendanceLedgerRepository').AttendanceLedgerRepository;
  ...
  get kakao_send_logs()       { return this.table('kakao_send_logs'); }
  get attendance_ledger()     { return this.table('attendance_ledger'); }
