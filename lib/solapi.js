 ...
 *
 * @see https://docs.solapi.com/api-reference/messages/send
 *
 * 사용 예:
...
class SolapiClient {
   ...
   * @param {string} path    - API 경로 (예: '/messages/v4/send')
   ...
   * @returns {Promise<Object>} { groupId, messageId, to, type, ... }
   */
  async _sendOne(message) {
    const body = { message };
    const result = await this._request('POST', '/messages/v4/send', body);

    // 응답에서 messageId 추출 (단건 발송 시)
    const msgInfo = result.message || result;
    return {
      ok: true,
      messageId: msgInfo.messageId,
      groupId: msgInfo.groupId || null,
      to: msgInfo.to,
      type: msgInfo.type,
      status: msgInfo.status || 'PENDING',
      raw: result,
    };
  }
   ...
   * @returns {Promise<Object>} { ok, groupId, count }
   */
  async sendGroupMessages(messages) {
  ...
  async getMessageStatus(messageId) {
    ...
    return {
      ...
      status: msg.status, // PENDING | SUCCESS | FAIL
      type: msg.type,
      to: msg.to,
