...
module.exports = { yieldEventLoop, batchedSync, batchedQuery, log, fmtLocalDate, getDisabledInappTypes };

/**
 * 인앱 알림 발송항목 설정 — 체크 해제(비활성)된 알림 유형 Set 반환
 * system_settings['inapp_notice_types_disabled'] JSON 배열 기준 (설정 없으면 빈 Set)
 */
function getDisabledInappTypes($) {
  try {
    const row = $.get(
      `SELECT setting_value FROM system_settings WHERE setting_key = ?`,
      ['inapp_notice_types_disabled']
    ) || {};
