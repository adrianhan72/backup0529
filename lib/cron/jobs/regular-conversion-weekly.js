 ...
 */
const { regularConversionWeekly, getMsgRuleFromContact, applyMsgRuleVars, contactFooter, CONTRACT_TYPE_KR } = require('../../message-templates');
...
module.exports = async function({ $, solapi, log, generateId }) {
    ...
    if (!rcEnabled) {
        log('[regular-conversion-weekly] 정규직 전환 고지 기능 OFF — 건너뜀');
        return 0;
    }

    // 인앱 알림 발송항목 설정 — 정규직 전환 체크 해제 시 건너뛰기
    if (getDisabledInappTypes($).has('regular_conversion')) {
        log('[regular-conversion-weekly] 인앱 알림 발송항목 체크 해제(정규직 전환) — 건너뜀');
        return 0;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        let title = `[재안내] 정규직 전환 의무 — ${r.emp_name} (누적 ${durationStr})`;
        let { body } = regularConversionWeekly(
            r.company_name, r.emp_name, r.first_start,
            durationStr, contactW
        );

        // ── 시스템 설정 본문 규칙 적용 (커스텀 규칙 있으면 덮어씀) ──
        const _rcwRule = getMsgRuleFromContact(contactW, 'regular_conversion');
        if (_rcwRule && (_rcwRule.title || _rcwRule.body)) {
            const _vars = {
                '회사명': r.company_name,
                '근로자명': r.emp_name,
            ...
            if (_rcwRule.title) title = applyMsgRuleVars(_rcwRule.title, _vars);
            if (_rcwRule.body) body = applyMsgRuleVars(_rcwRule.body, _vars) + '\n\n' + contactFooter(contactW);
        }
