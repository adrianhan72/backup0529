...
class BillingRepository extends BaseRepository {
  ...
  findUnpaid() {
    return this.db.prepare(
      `SELECT * FROM billing WHERE payment_status != '${PAYMENT_STATUS.PAID}' ORDER BY due_date`
    ).all();
  }
  ...
  /** 납부 완료 처리 */
  markPaid(id) {
    return this.patch(id, { payment_status: PAYMENT_STATUS.PAID, updated_at: Date.now() });
  }
  ...
  sumByYearMonth(year, month) {
    return this.db.prepare(`
      SELECT
        ...
        SUM(CASE WHEN payment_status = '${PAYMENT_STATUS.PAID}' THEN total_amount ELSE 0 END) AS paid_amount,
        SUM(CASE WHEN payment_status != '${PAYMENT_STATUS.PAID}' THEN total_amount ELSE 0 END) AS unpaid_amount
      FROM billing
