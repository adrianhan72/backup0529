...
class ContractDispatchRepository extends BaseRepository {
  ...
  countPending(companyId = null) {
    const sql = companyId
      ? `SELECT COUNT(*) AS cnt FROM contract_dispatch WHERE company_id = ? AND dispatch_status = '${DISPATCH_STATUS.PENDING}'`
      : `SELECT COUNT(*) AS cnt FROM contract_dispatch WHERE dispatch_status = '${DISPATCH_STATUS.PENDING}'`;
