/**
 * company/company-history.mjs — Phase 6: 완전 변환
...
export function _fmtHistVal(val, field){
...
export function _renderCompanyHistory(companyId){
  const sec = document.getElementById('cm-history-section');
  if(!sec) return;
  const rows = (window.allCompanyHistories||[])
 ...
 */
export function getCompanySnapshotAt(companyId, contractTimestamp){
  const co = (getCompanies()||[]).find(x=>x.id===companyId);
  if(!co) return null;
  const hist = (window.allCompanyHistories||[])
...
export function cmAwTogglePayType(field, checked){
...
export function _cmAwHtmlId(f){ return f.replace(/_/g, '-'); }

/** 모달 → allowance_config 객체 수집 */
export function _cmGetAllowanceConfig(){
  const cfg = {};
...
export function _cmSetAllowanceConfig(cfg){
  if(!cfg) cfg = {};
  _CM_AW_SIMPLE_FIELDS.forEach(f => {
...
// ── 대시보드 카드용 사용료 현황 계산 ──
export function getBillingInfoForDashCard(companyId){
  ...
  // 이번 달 실제 청구
  const thisBill = getPayrolls().find(b =>
...
export function goBillingByCompany(companyId, companyName){
...
export function clearBillingCompanyFilter(){
...
// window.currentContCompanyId → 최상단 STATE 블록에서 선언됨

// 고객사 칩 목록 렌더링
export function renderContCompanyList(){
  ...
  container.innerHTML = companies.map(c => {
    const isSelected = c.id === window.currentGlobalCompanyId;
    // 오늘 현재 유효한 계약 (활성 + 서류미비, 임시저장 제외)
    const activeContracts = getContracts().filter(ct => ct.company_id === c.id && !ct.is_draft && (ct.status === '활성' || ct.status === 'active' || ct.status === '서류미비' || ct.status === 'docs_incomplete'));
    // 유효 계약을 가진 고유 근로자 수
...
// 근로계약서 페이지 고객사 선택
export function selectContCompany(companyId, companyName){
...
// 고객사 선택 해제
export function clearContCompanySelect(){
...
export function goContractsByCompany(companyId, companyName){
  const co = getCompanies().find(c => c.id === companyId);
  if(co && !isCompanyActive(co)){
    _w('toast')('"' + companyName + '"은 이용중이 아닌 고객사입니다. 근로계약 관리가 불가합니다.', 'error');
    return;
  }
...
export function goLaborStatusByCompany(companyId, companyName){
...
export function clearContractCompanyFilter(){ clearContCompanySelect(); }

// ─── 임금대장 고객사 칩 선택 ───
// window.currentPayCompanyId → 최상단 STATE 블록에서 선언됨

export function renderPayCompanyList(){
  ...
  container.innerHTML = companies.map(c => {
    const isSelected = c.id === window.currentGlobalCompanyId;
    const payCnt = getPayrolls().filter(p => !p.is_draft && p.company_id === c.id).length;
...
export function selectPayCompany(companyId, companyName){
...
export function clearPayCompanySelect(){
...
// 연도·월 필터 초기화
export function initPayYearMonth(){
...
export function goPayrollsByCompany(companyId, companyName){
  const co = getCompanies().find(c => c.id === companyId);
  if(co && !isCompanyActive(co)){
    _w('toast')('"' + companyName + '"은 이용중이 아닌 고객사입니다. 급여 조회가 불가합니다.', 'error');
    return;
  }
...
// 하위 호환
export function showPayrollCompanyBanner(){}
export function clearPayrollCompanyFilter(){ clearPayCompanySelect(); }

// 해지 대상 고객사 임시 저장
let terminateTargetId = null;

// [사용료 숨김] terminateCompany - 단순화 버전 (미납금 체크 제거)
export function terminateCompany(id, name){
  ...
  _w('openModal')('terminate-modal');
}

// [사용료 숨김] doTerminate - 단순화 버전 (미납금 체크 없이 즉시 해지)
export async function doTerminate(){
  ...
  if(!endDateStr) return _w('toast')('계약 해지일을 입력해 주세요.', 'error');
  const todayStr = new Date().toISOString().slice(0, 10);
  ...
  await _w('api')(`../tables/companies/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(patch)});
  _w('closeModal')('terminate-modal');
...
export function changeEndDate(id, name, currentEndDate){
  ...
  _w('openModal')('change-end-date-modal');
}

export async function doChangeEndDate(){
  ...
  if(!newDateStr) return _w('toast')('해지일을 선택해 주세요.', 'error');
  const todayStr = new Date().toISOString().slice(0,10);
  ...
  await _w('api')(`../tables/companies/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(patch)});
  _w('closeModal')('change-end-date-modal');
  await loadCompanies();
  populateFilters(); populatePICompanies(); renderCompanies(); _w('renderDashboard')();
...
// ── 해지예정 고객사 — 해지 취소 ──
export async function cancelTerminate(id, name){
  const c = getCompanies().find(x => x.id === id);
 ...
 */
export async function _createTermNotice(company, endDateStr, oldEndDateStr){
  if(!company) return;
  const adminName = typeof _getAdminUsername === 'function' ? _w('_getAdminUsername')() : '관리자';
 ...
 */
export async function _createCancelNotice(company){
  if(!company) return;
  const adminName = typeof _getAdminUsername === 'function' ? _w('_getAdminUsername')() : '관리자';
...
export function terminateCompany_DISABLED(id, name){
  const todayStr = new Date().toISOString().slice(0, 10);
  terminateTargetId = id;
  let unpaidAmount = 0, unpaidCount = 0;
  let pendingAmount = 0, pendingCount = 0;
  getPayrolls().filter(b => b.company_id === id).forEach(b => {
...
export async function doTerminate_DISABLED(withLoss){
  const id = terminateTargetId;
  if(!id) return;
  const c = getCompanies().find(x => x.id === id);
  if(!c) return;
  const todayStr = new Date().toISOString().slice(0, 10);
  if(!withLoss){
    const hasBalance = getPayrolls().some(b => {
    ...
    if(hasBalance){
      _w('closeModal')('terminate-modal');
      _w('toast')('완납 후 해지할 수 있습니다. 미정산 금액을 먼저 처리해 주세요.', 'error');
      return;
    }
    const body = {...c, status: CONTRACT_STATUS.TERMINATED, contract_end_date: todayStr};
    await _w('api')(`../tables/companies/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    _w('closeModal')('terminate-modal');
    await loadCompanies();populateFilters();populatePICompanies();renderCompanies();_w('renderDashboard')();
    _w('toast')(`"${c.company_name}" 해지 완료 (해지일: ${todayStr})`);
    return;
  }
  let totalLoss = 0;
  const unpaidBillings = getPayrolls().filter(b => {
    if(b.company_id !== id || b.payment_status ===PAYMENT_STATUS.PAID) return false;
    const rem = (b.total_amount || 0) - (b.partial_paid_amount || 0);
    return rem > 0;
  });
  for(const b of unpaidBillings){
    ...
    await _w('api')(`../tables/billings/${b.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(updBody)});
  }
  ...
  await _w('api')(`../tables/companies/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(compBody)});
  _w('closeModal')('terminate-modal');
  await loadData();populateFilters();populatePICompanies();renderCompanies();_w('renderDashboard')();renderBillingTrendChart();
  _w('toast')(`"${c.company_name}" 손실 처리(${Math.round(totalLoss).toLocaleString('ko-KR')}원) 후 해지 완료`);
}
*/


// ══ window 등록 (레거시 호환) ══
window._fmtHistVal = _fmtHistVal;
window._renderCompanyHistory = _renderCompanyHistory;
window._cmHistGoPage = _cmHistGoPage;
window._cmHistToggle = _cmHistToggle;
window.getCompanySnapshotAt = getCompanySnapshotAt;
window._parseCo = _parseCo;
window.cmAwTogglePayType = cmAwTogglePayType;
window._cmAwHtmlId = _cmAwHtmlId;
window._cmGetAllowanceConfig = _cmGetAllowanceConfig;
window._cmSetAllowanceConfig = _cmSetAllowanceConfig;
window.getBillingInfoForDashCard = getBillingInfoForDashCard;
window.goBillingByCompany = goBillingByCompany;
window.clearBillingCompanyFilter = clearBillingCompanyFilter;
window.renderContCompanyList = renderContCompanyList;
window.selectContCompany = selectContCompany;
window.clearContCompanySelect = clearContCompanySelect;
window.goContractsByCompany = goContractsByCompany;
window.goLaborStatusByCompany = goLaborStatusByCompany;
window.clearContractCompanyFilter = clearContractCompanyFilter;
window.renderPayCompanyList = renderPayCompanyList;
window.selectPayCompany = selectPayCompany;
window.clearPayCompanySelect = clearPayCompanySelect;
window.initPayYearMonth = initPayYearMonth;
window.goPayrollsByCompany = goPayrollsByCompany;
window.showPayrollCompanyBanner = showPayrollCompanyBanner;
window.clearPayrollCompanyFilter = clearPayrollCompanyFilter;
window.terminateCompany = terminateCompany;
window.doTerminate = doTerminate;
window.changeEndDate = changeEndDate;
window.doChangeEndDate = doChangeEndDate;
