/**
 * wage-ledger/severance-full.mjs — Phase 6: 퇴직금
...
window._sevCompanyId = null;
window._sevCompanyName = '';
let _sevCurrentRecord = null; // 현재 팝업에 열린 퇴직금 정산 데이터

// ── 고객사 목록 렌더 ──
export function renderSevCompanyList(){
  ...
  const list = getCompanies().filter(c => !c.is_draft && (c.status===COMPANY_STATUS.ACTIVE||c.status===COMPANY_STATUS.INACTIVE) &&
...
// ── 고객사 선택 ──
export function selectSevCompany(id, name){
  _sevCompanyId = id;
...
// ── 고객사 선택 해제 ──
export function clearSevCompanySelect(){
  _sevCompanyId = null;
  _sevCompanyName = '';
...
// ── 탭 전환 ──
export function switchSevTab(tab){
...
// ── 재직 기간 계산 (년·개월·일 텍스트) ──
export function calcTenure(hireDate, baseDate){
...
export function getPrev3MonthsPayrolls(empId, baseDate){
...
export function calcOrdinaryWage3(pays){
...
export function calcAverageWage3(pays){
...
export function calcSeverancePay(empId, hireDate, baseDate, pays3){
...
// ── 계약 상태 뱃지 HTML ──
export function sevStatusBadge(emp, contract){
...
// ── 메인 렌더 (탭1 + 탭2 모두) ──
export function renderSeverance(){
...
// ── 탭1: 근로자별 퇴직급여 현황 ──
export function renderSevStatusTab(){
  ...
  emps.sort((a,b)=>(a.name||'').localeCompare(b.name||'','ko')).forEach(emp => {
    ...
    const empContracts = getContracts()
    ...
    rows.push(`
      <tr style="border-top:2px solid #fde68a;">
        ...
        <td rowspan="${mainRowspan}" style="padding:10px 10px;color:#374151;vertical-align:top;border-right:1px solid #fef3c7;">${_w('contractTypeLabel')(emp.employment_category) || '-'}</td>
...
// ── 탭2: 지급 발생 이력 ──
export function renderSevHistoryTab(){
...
// ══ window 등록 (레거시 호환) ══
window.renderSevCompanyList = renderSevCompanyList;
window.selectSevCompany = selectSevCompany;
window.clearSevCompanySelect = clearSevCompanySelect;
window.switchSevTab = switchSevTab;
window.calcTenure = calcTenure;
window.getPrev3MonthsPayrolls = getPrev3MonthsPayrolls;
window.calcOrdinaryWage3 = calcOrdinaryWage3;
window.calcAverageWage3 = calcAverageWage3;
window.calcSeverancePay = calcSeverancePay;
window.sevStatusBadge = sevStatusBadge;
window.renderSeverance = renderSeverance;
window.renderSevStatusTab = renderSevStatusTab;
window.renderSevHistoryTab = renderSevHistoryTab;
