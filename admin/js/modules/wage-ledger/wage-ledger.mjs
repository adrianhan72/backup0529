/**
 * wage-ledger/wage-ledger.mjs — Phase 6: 임금대장
...
export function updateMenuBadges(){
  ...
  const draftCompanyCount = (getCompanies()||[]).filter(c => !!c.is_draft).length;
  ...
  // 그 전에 호출되면 _contractDispatchList·_allSendLogs·getPayrolls() 등이 빈 배열 상태이므로
  ...
  if(typeof _cenHistoryLoaded !== 'undefined' && !_cenHistoryLoaded){
    // loadHeavyData() 완료 후 _w('updateMenuBadges')() 재호출 시 올바른 값으로 갱신됨
  } else {
  ...
  // getContracts() 기반 계산 → critical path 완료 후 정확 (heavy 데이터 불필요)
  ...
  // getPayrolls()는 heavy 데이터이므로 로드 완료 후에만 정확히 집계
  if(_heavyReady){
    const draftPayrollCount = (getPayrolls()||[]).filter(p => !!p.is_draft).length;
    _setBadge('badge-payroll-input', draftPayrollCount);
  }
}

// ── 임금대장 메뉴 뱃지 업데이트 ──
export function _updateWLMenuBadge(){
...
export async function _checkWageLedgerComplete(companyId, year, month){
  // 열람 대상 월의 첫날·말일
  ...
  const validContracts = getContracts().filter(ct => {
  ...
  const checkRes = await _w('api')(`../tables/wage_ledger_notifications?limit=500`);
  ...
  if(alreadyExists){
    ...
    await _w('api')(`../tables/wage_ledger_notifications/${alreadyExists.id}`,{
      method:'PATCH',
    ...
    // 신규 알림 생성
    await _w('api')('../tables/wage_ledger_notifications',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        id:'wln'+Date.now(),
  ...
  // 캐시 갱신 + 뱃지 업데이트
  await _w('loadWLNotifications')();
  _w('_updateWLMenuBadge')();
  ...
  _w('toast')(`📋 ${co?.company_name||''} ${year}년 ${month}월 임금대장이 생성되었습니다!`, 'success');
}

export function renderWLCompanyList(){
...
export function selectWLCompany(id, name){
  _wlCompanyId = id;
  _wlCompanyName = name;
...
export async function _markWLNotificationsRead(companyId){
  ...
  await Promise.all(unread.map(n =>
    _w('api')(`../tables/wage_ledger_notifications/${n.id}`,{
      method:'PATCH',
      headers:{'Content-Type':'application/json'},
...
export function _wlGoToPayInput(empId){
  ...
  const draftRec = (getPayrolls() || []).find(p =>
    p.employee_id === empId &&
...
export function _wlNavigateToPayInput(empId, yr, mo, resume){
  ...
  setTimeout(() => {
    ...
    // 고객사 자동 선택
    const co = getCompanies().find(c => c.id === _wlCompanyId);
...
export function clearWLCompanySelect(){
  _wlCompanyId = null;
  _wlCompanyName = '';
...
export function _initWLFilters(){
  const now = new Date();
...
export function _populateWLMonthOptions(year){
  const moSel = document.getElementById('wl-month-filter');
  if(!moSel) return;
  const now = new Date();
  const curYr = now.getFullYear();
...
export function _setWLFilterReady(ready){
  const yrSel = document.getElementById('wl-year-filter');
  const moSel = document.getElementById('wl-month-filter');
...
export function renderWageLedger(){
  const area = document.getElementById('wl-table-area');
  ...
  // 해당 월 급여 데이터
  const pays = getPayrolls().filter(p =>
  ...
  // 해당 고객사의 모든 계약 목록
  const coContracts = getContracts().filter(c => c.company_id === _wlCompanyId);
  ...
  if(hasBlock){
    ...
    const _empInfo = id => {
      const e = getEmployees().find(x => x.id === id);
      ...
      <div class="wl-block-item-list">
        ${docsIncomplete.map(c => {
          ...
          return `<div class="wl-block-item">
            ...
            <button class="wl-block-item-link docs" onclick="_w('viewContract')('${c.id}')">
  ...
  // 직원 정보 매핑
  const empMap = {};
  getEmployees().forEach(e => { empMap[e.id] = e; });
  ...
  const buildTbody = (p, emp, idx, isSum) => {
    const resign = (getContracts().filter(c => c.employee_id === (p?.employee_id||'') && !c.is_draft)
...
export function _wlDoPrint(isPdf){
...
// ─── 임금대장 PDF 다운로드 ───
export function downloadWageLedgerPdf(){
  if(!_wlCompanyId){ _w('toast')('고객사를 먼저 선택해 주세요.','warning'); return; }
  _wlDoPrint(true);
}

// ─── 임금대장 인쇄 ───
export function printWageLedger(){
  if(!_wlCompanyId){ _w('toast')('고객사를 먼저 선택해 주세요.','warning'); return; }
  _wlDoPrint(false);
}
...
export function downloadWageLedgerExcel(){
  if(!_wlCompanyId){ _w('toast')('고객사를 먼저 선택하세요.','error'); return; }

  const yr    = parseInt(document.getElementById('wl-year-filter')?.value);
  const mo    = parseInt(document.getElementById('wl-month-filter')?.value);
  const moStr = String(mo).padStart(2,'0');

  let pays = getPayrolls().filter(p =>
    p.company_id === _wlCompanyId &&
    Number(p.pay_year) === yr &&
    Number(p.pay_month) === mo
  );
  if(!pays.length){ _w('toast')(`${yr}년 ${mo}월 급여 데이터가 없습니다.`,'error'); return; }
  ...
  _w('toast')(`📥 ${yr}년 ${mo}월 임금대장 엑셀 다운로드 완료`, 'success');
}


// ══ window 등록 (레거시 호환) ══
window.updateMenuBadges = updateMenuBadges;
window._updateWLMenuBadge = _updateWLMenuBadge;
window._checkWageLedgerComplete = _checkWageLedgerComplete;
window.renderWLCompanyList = renderWLCompanyList;
window.selectWLCompany = selectWLCompany;
window._markWLNotificationsRead = _markWLNotificationsRead;
window._wlGoToPayInput = _wlGoToPayInput;
window.closeWLResumeModal = closeWLResumeModal;
window._wlResumeDraft = _wlResumeDraft;
window._wlFreshInput = _wlFreshInput;
window._wlNavigateToPayInput = _wlNavigateToPayInput;
window.clearWLCompanySelect = clearWLCompanySelect;
window._initWLFilters = _initWLFilters;
window._populateWLMonthOptions = _populateWLMonthOptions;
window._setWLFilterReady = _setWLFilterReady;
window.renderWageLedger = renderWageLedger;
window._wlDoPrint = _wlDoPrint;
window.downloadWageLedgerPdf = downloadWageLedgerPdf;
window.printWageLedger = printWageLedger;
window.downloadWageLedgerExcel = downloadWageLedgerExcel;
