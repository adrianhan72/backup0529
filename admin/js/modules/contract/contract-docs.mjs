...
export function _renderContractFilesSection(c){
...
export function _ctfMakeRow(type, c, label, icon, color, bgColor, isVoidedFile=false){
...
export function _ctfTogglePreview(prevId, btn){
...
// ── 파일 업로드 ──
export async function _ctfUpload(type, contractId, inputEl){
  const file = inputEl.files[0];
  if(!file) return;

  const MAX = 10 * 1024 * 1024; // 10MB
  if(file.size > MAX){ _w('toast')('파일 크기는 10MB 이하만 허용됩니다.', 'error'); inputEl.value=''; return; }
  ...
  try{
    ...
    // 로컬 캐시 갱신
    const idx = getContracts().findIndex(x => x.id === contractId);
    if(idx !== -1){ getContracts()[idx][nameField] = file.name; getContracts()[idx][dataField] = base64; }
    ...
    if(c && (c.status === 'docs_incomplete' || c.status === '서류미비')){
      if(c.signed_file_data && c.consent_file_data){
        ...
        if(idx !== -1) getContracts()[idx].status = 'active';
      }
    }

    _w('toast')('파일이 업로드되었습니다.', 'success');
    // 섹션 재렌더링
    ...
    if(typeof renderContracts === 'function') _w('renderContracts')();

    // ── 고객사 인앱 알림 발송 (날인본/동의서 업로드) ──
    {
      const _ufCo  = getCompanies().find(x => x.id === c?.company_id) || {};
      const _ufEmp = getEmployees().find(x => x.id === c?.employee_id) || {};
      const _coRep = _w('getCompanyRepGreeting')(_ufCo);
...
${window._BRAND_SIG}`,
        ...
        const _updatedC = getContracts().find(x => x.id === contractId);
...
// ── 파일 삭제 ──
export async function _ctfDelete(type, contractId){
  ...
  try{
    ...
    _w('toast')(`'${label}' 파일이 삭제되었습니다.`, 'success');
    const c = getContracts().find(x => x.id === contractId);
    if(c) _renderContractFilesSection(c);
    // 업로드 모달도 갱신
    if(document.getElementById('contract-preview-modal')?.classList.contains('open')){
...
// ── 파일 다운로드 ──
export function _ctfDownload(type){
...
// ── File → Base64 변환 ──
export function _ctfToBase64(file){
...
export function _renderCpExistingFiles(c){
  if(!c) return;
...
// ── 개별 파일 행 DOM 생성 ──
export function _cpCreateFileRow(c, type, label, icon, color, bgColor, isVoidedFile){
  const nameField = type === 'signed' ? 'signed_file_name'  : 'consent_file_name';
  const dataField = type === 'signed' ? 'signed_file_data'  : 'consent_file_data';
  const hasFile   = !!(c[dataField]);
  const fileName  = c[nameField] || '첨부파일';
...
// ── 버튼 생성 헬퍼 ──
export function _cpCreateBtn(text, className, iconClass, clickHandler){
...
export function _renderCpRegistrationUpload(){
  const container = document.getElementById('cp-files-container');
  if(!container) return;
  container.innerHTML = `
    <!-- 계약서 날인본 -->
...
export function _downloadContractFile(encodedData, encodedName){
  const a = document.createElement('a');
...
// 단계바 상태 업데이트
export function setContractStep(step){
  [1,2,3].forEach(n=>{
...
// 폼 유효성 간단 검사
// 계약서 미리보기 모달 열기
export async function openContractPreview(){
  _resetUploadState();
  ...
  // 바로 저장
  setContractStep(3);
  if(typeof saveContract !== 'function'){ _w('toast')('saveContract 함수를 찾을 수 없습니다.', 'error'); return; }
  try { await saveContract(); } catch(e) {
    console.error('[openContractPreview] saveContract 오류:', e);
    _w('toast')('저장 중 오류: ' + (e.message || '알 수 없는 오류'), 'error');
  }
}

export function closeContractPreview(){
  ...
  if(window._cpUploadOnly){
    ...
    if(typeof loadContracts === 'function') _w('loadContracts')().then(() => {
      if(typeof renderContracts === 'function') _w('renderContracts')();
    });
  }
}

export function showCpTab(tab){
...
// 최종 등록 버튼 활성화 조건 체크
export function _updateFinalBtn(){
...
// ── 계약서 날인본 핸들러 ──
export function handleContractFileSelect(e){
  const file = e.target.files[0];
  if(file) _setContractFile(file);
}
export function handleContractFileDrop(e){
  e.preventDefault();
  e.currentTarget.style.borderColor = '';
  const file = e.dataTransfer.files[0];
  if(file) _setContractFile(file);
}
export function _setContractFile(file){
...
export function clearContractUpload(){
...
// ── 제3자 개인정보 제공 동의서 핸들러 ──
export function handleConsentFileSelect(e){
  const file = e.target.files[0];
  if(file) _setConsentFile(file);
}
export function handleConsentFileDrop(e){
  e.preventDefault();
  e.currentTarget.style.borderColor = '';
  const file = e.dataTransfer.files[0];
  if(file) _setConsentFile(file);
}
export function _setConsentFile(file){
...
export function clearConsentUpload(){
...
export function _resetUploadState(){
  window._contractSignedFile  = null;
  window._contractConsentFile = null;
  // UI 초기화
...
export async function finalSaveContract(){
  setContractStep(3);
  document.getElementById('contract-preview-modal').classList.remove('open');
  try { await saveContract(); } catch(e){ console.error('[finalSaveContract] saveContract 오류:', e); _w('toast')('저장 중 오류가 발생했습니다: ' + e.message, 'error'); }
}
...
// ── WORD(docx) 다운로드 ──
export function downloadContractDocx(){
...
export function _buildDocx(){
    ...
    sections:[{
      ...
      children:[
        ...
        mkTable([
          ...
          infoRow('고용형태', _w('contractTypeLabel')(data.contractType)||data.contractType),
        ]),
  ...
  Packer.toBlob(doc).then(blob=>{
    ...
    _w('toast')('WORD 파일이 다운로드됐습니다. (고용노동부 표준 양식)');
  });
}

// ── 폼 데이터 수집 ──
export function _collectContractData(){
  ...
  const company = (coId && typeof getCompanySnapshotAt === 'function')
    ? (getCompanySnapshotAt(coId, _ctTsForSnap) || getCompanies().find(c=>c.id===coId) || {})
    : (getCompanies().find(c=>c.id===coId)||{});
  ...
  if(isEdit){
    const existC = getContracts().find(x=>x.id===editId.contract);
    const existE = existC ? getEmployees().find(e=>e.id===existC.employee_id) : null;
  ...
  return {
    ...
    representative:    _w('getCompanyRepName')(company),
...
export function generateContractHTML(){
  ...
  <div class="doc-section">
    ...
    <table class="info-table">
      ...
      ${row('고용형태', _w('contractTypeLabel')(d.contractType)||d.contractType)}
 ...
 * getContracts() / getEmployees() / getCompanies() 전역 배열로 데이터 수집
 */
export function openContractPrintModal(contractId){
  if(!contractId){ _w('toast')('계약 정보를 찾을 수 없습니다.'); return; }
  const c   = getContracts().find(x=>x.id===contractId);
  const emp = c ? getEmployees().find(e=>e.id===c.employee_id) : null;
  ...
  const co  = c ? (getCompanySnapshotAt(c.company_id, _printCtTs) || getCompanies().find(x=>x.id===c.company_id)) : null;
  if(!c||!emp||!co){ _w('toast')('계약·직원·고객사 정보를 불러올 수 없습니다.'); return; }
  ...
  let html;
  try{
    html = _w('generateContractHTMLFromData')(c, emp, co);
  } catch(err){
    console.error('[계약서 생성 오류] contractId:', contractId, err);
    _w('toast')('계약서 생성 중 오류가 발생했습니다. 계약 데이터를 확인해 주세요.\n(' + err.message + ')');
    return;
  }
 ...
 */
export async function _updateCpmGuide(contractId, isVoided, voidedAt){
  ...
  try {
    await _w('loadContractDispatchList')();
...
export function closeContractPrintModal(){
...
export function openContractInNewWindow(contractId){
  if(!contractId){ _w('toast')('계약 정보를 찾을 수 없습니다.'); return; }
  const c   = getContracts().find(x => x.id === contractId);
  const emp = c ? getEmployees().find(e => e.id === c.employee_id) : null;
  const co  = c ? getCompanies().find(x => x.id === c.company_id) : null;
  if(!c || !emp || !co){ _w('toast')('계약·직원·고객사 정보를 불러올 수 없습니다.'); return; }
  ...
  try {
    ...
    _w('toast')('계약서 생성 중 오류: ' + err.message);
    return;
  }

  const css = _w('_getContractPrintCSS')();
  const win = window.open('', '_blank', 'width=1000,height=820');
  if(!win){ _w('toast')('팝업이 차단되었습니다. 브라우저 팝업 허용 후 다시 시도해 주세요.'); return; }
...
/** 계약서 인쇄 (새 창) */
export function printContractDoc(){
...
export async function downloadContractPdf(){
  ...
  try {
    ...
    _w('toast')('PDF가 저장되었습니다.');
  } catch(e){
    console.error('[계약서 PDF]', e);
    _w('toast')('PDF 생성 중 오류가 발생했습니다. 인쇄(Ctrl+P)를 이용해 PDF로 저장하세요.');
  } finally {
...
// ══ window 등록 (레거시 호환) ══
window._renderContractFilesSection = _renderContractFilesSection;
window._ctfMakeRow = _ctfMakeRow;
window._ctfTogglePreview = _ctfTogglePreview;
window._ctfUpload = _ctfUpload;
window._ctfDelete = _ctfDelete;
window._ctfDownload = _ctfDownload;
window._ctfToBase64 = _ctfToBase64;
window._renderCpExistingFiles = _renderCpExistingFiles;
window._cpCreateFileRow = _cpCreateFileRow;
window._cpCreateBtn = _cpCreateBtn;
window._renderCpRegistrationUpload = _renderCpRegistrationUpload;
window._downloadContractFile = _downloadContractFile;
window.setContractStep = setContractStep;
window.openContractPreview = openContractPreview;
window.closeContractPreview = closeContractPreview;
window.showCpTab = showCpTab;
window._updateFinalBtn = _updateFinalBtn;
window.handleContractFileSelect = handleContractFileSelect;
window.handleContractFileDrop = handleContractFileDrop;
window._setContractFile = _setContractFile;
window.clearContractUpload = clearContractUpload;
window.handleConsentFileSelect = handleConsentFileSelect;
window.handleConsentFileDrop = handleConsentFileDrop;
window._setConsentFile = _setConsentFile;
window.clearConsentUpload = clearConsentUpload;
window._resetUploadState = _resetUploadState;
window._fileToBase64 = _fileToBase64;
window.finalSaveContract = finalSaveContract;
window.printContract = printContract;
window.downloadContractDocx = downloadContractDocx;
window._buildDocx = _buildDocx;
window._collectContractData = _collectContractData;
window.generateContractHTML = generateContractHTML;
window.openContractPrintModal = openContractPrintModal;
window._updateCpmGuide = _updateCpmGuide;
window.closeContractPrintModal = closeContractPrintModal;
window.openContractInNewWindow = openContractInNewWindow;
window.printContractDoc = printContractDoc;
