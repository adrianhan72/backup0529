/**
 * contract/contract-form.mjs — Phase 6: 계약 양식
...
const _BRAND_SIG = '─────────────────────\n인사톡 노무톡 · 대화인사노무파트너스';
window._BRAND_SIG = _BRAND_SIG;

// ─── EMPLOYEES ───
// ─── CONTRACTS ───
export function toggleEmExpire(){
...
export function _checkFixedTermDuration(){
  // ── 신규 모드 경고 박스
...
export function calcAnnualLeaveDays(hireDateStr, basisType, contractStartStr){
...
export function _autoFillCTPeriod(){
...
export function _ctPeriodCompose(){
...
export function _ctPeriodRestore(payPeriod, month, day){
...
export function toggleProbation(){
  const rawCat = document.getElementById('ct-em-category').value;
  const cat = CONTRACT_TYPE_LEGACY_MAP[rawCat] || rawCat;
...
// ── 산정기준 라디오 변경 핸들러 ──
export function onProbationBasisChange(){
...
export function getProbationBase(){
  const basis = document.querySelector('input[name="ct-probation-basis"]:checked')?.value || 'salary';
  if(basis === 'minwage'){
...
export function calcProbationFromPct(){
  const basis = document.querySelector('input[name="ct-probation-basis"]:checked')?.value || 'salary';
...
export function calcProbationFromAmt(){
  const basis = document.querySelector('input[name="ct-probation-basis"]:checked')?.value || 'salary';
  if(basis !== 'direct'){
...
export function _checkProbMinWageWarning(){
...
// ──────────────────────────────────────────────────────────────────────────────
export function _checkMinWageWarning(){
  ...
  if(isDaily){
    const dw   = _w('getAmountVal')('ct-daily-wage');
    ...
    const pos    = _w('getAmountVal')('ct-position');
    const car    = _w('getAmountVal')('ct-car');
    const rmtArea= _w('getAmountVal')('ct-remote-area')||0;
    const meal   = _w('getAmountVal')('ct-meal');
    const res    = _w('getAmountVal')('ct-research');
    const other  = _w('getAmountVal')('ct-other') || 0;
    const site_w = _w('getAmountVal')('ct-site')||0;
    const skill_w= _w('getAmountVal')('ct-skill')||0;
    const lic_w  = _w('getAmountVal')('ct-license')||0;
    const comm_w = _w('getAmountVal')('ct-communication')||0;
    const fit_w  = _w('getAmountVal')('ct-fitness')||0;
    const sdev_w = _w('getAmountVal')('ct-self-dev')||0;
    const book_w = _w('getAmountVal')('ct-book')||0;
    const ovs_w  = _w('getAmountVal')('ct-overseas')||0;
...
export function _checkRegisterBtnState(){ /* no-op: 버튼 항상 활성 */ }
export function _checkAmendBtnState(){    /* no-op: 버튼 항상 활성 */ }

export function syncProbation(){
  // 수습 섹션이 보이는 경우에만 계산
  const sec = document.getElementById('ct-probation-section');
...
// ── 일괄 설정 요일 체크박스 연동 ──
export function _bulkWeekdayToggle(){
...
export function _bulkDayToggle(){
...
// ── 일괄 설정 적용 ──
export function applyBulkSchedule(){
  ...
  if(!start || !end){ _w('toast')('출근·퇴근 시간을 입력해 주세요.', 'error'); return; }
  if(start >= end){ _w('toast')('퇴근 시간이 출근 시간보다 늦어야 합니다.', 'error'); return; }
  if(brks && brke && brks >= brke){ _w('toast')('휴게 종료 시간이 시작 시간보다 늦어야 합니다.', 'error'); return; }
  ...
  if(targets.size === 0){ _w('toast')('적용할 요일을 하나 이상 선택해 주세요.', 'error'); return; }
  ...
  calcWorkHours();
  _w('toast')(`${applied}개 요일에 근무시간이 일괄 적용되었습니다. ✔`, 'success');
}

// ── 시프트 그룹 렌더 헬퍼 ──
export function _shiftGroupHTML(key, idx, enabled, start, end, breaks){
...
// ── 첫 번째 시프트 비활성화 ──
export function _deactivateShift(key){
  const container = document.getElementById(`ct-sch-shifts-${key}`);
  if(!container) return;
  container.innerHTML = _shiftGroupHTML(key, 0, false, '', '', []);
  calcWorkHours();
...
export function _brkSlotsHTML2(key, shiftIdx, enabled, breaks){
  const dis = enabled ? '' : 'disabled';
...
// ── 시프트 삭제 ──
export function _removeShift(key, idx){
  const sid = idx===0 ? '' : '-'+idx;
...
// ── 시프트용 휴게 슬롯 추가 ──
export function _addBrkSlot2(key, shiftIdx){
  const sid = shiftIdx===0 ? '' : '-'+shiftIdx;
...
export function _removeBrkSlot2(key, shiftIdx, idx){
  const sid = shiftIdx===0 ? '' : '-'+shiftIdx;
  const wrap = document.getElementById(`ct-sch-brkwrap-${key}${sid}`);
  if(!wrap) return;
...
export function _getBrkSlots2(key, shiftIdx){
  const sid = shiftIdx===0 ? '' : '-'+shiftIdx;
  const wrap = document.getElementById(`ct-sch-brkwrap-${key}${sid}`);
  if(!wrap) return [];
...
export function _getBrkSlots(key){
...
export function _addBrkSlot(key){
  const wrap = document.getElementById(`ct-sch-brkwrap-${key}`);
  if(!wrap) return;
  const idx = wrap.querySelectorAll('.brk-slot-row').length;
  const div = document.createElement('div');
...
export function _removeBrkSlot(key, idx){
  const wrap = document.getElementById(`ct-sch-brkwrap-${key}`);
  if(!wrap) return;
...
export function _setBrkSlots(key, breaks, enabled){
  const wrap = document.getElementById(`ct-sch-brkwrap-${key}`);
  if(!wrap) return;
...
export function initScheduleTable(){
...
// 스케줄 → JSON 직렬화 (저장용)
export function getScheduleJSON(){
...
// JSON → 스케줄 UI 복원
export function setScheduleFromJSON(schedule){
...
export function setScheduleFromLegacy(c){
...
export function initBreakSelects(){ initScheduleTable(); }
export function getBreakMins(hId,mId){ return 0; }
export function setBreakMins(hId,mId,totalMins){}
export function toggleCtEndDate(preserveValue=false){
...
export function setCTPayType(field, type){
...
export function _getCTPayTypeVal(field){ return _ctPayTypes[field] || 'fixed'; }
// 해당 수당이 월 약정임금 합산 대상인지 반환 (fixed = 매월 정기지급 = 포함, 그 외 제외)
export function _isFixedAllow(field){ return _getCTPayTypeVal(field) === 'fixed'; }
export function _resetCTPayTypes(){
...
export function applyCTAllowanceConfig(cfg, clearValues = false){
 ...
 */
export function onCtStartChange(){
  ...
  if(isNewMode){
    ...
    const co = (getCompanies() || []).find(x => x.id === coId);
 ...
 */
export function onCtCompanyChange(){
  // 조회 전용(ct-readonly) 상태에서는 동작하지 않음
  const modalEl = document.querySelector('#contract-modal .modal');
  const isReadonly = modalEl && modalEl.classList.contains('ct-readonly');
  if(isReadonly) return;
...
export function _setCtPayDayDefault(coId){
...
export function _forceShowNonZeroCTRows(c){
  const _fieldMap = {
 ...
 */
export function _calcMonthlyStdHours(hpd, dpw){
...
export function calcContractSalary(){
  ...
  const position   = _w('getAmountVal')('ct-position');
  const car        = _w('getAmountVal')('ct-car');
  const remoteArea = _w('getAmountVal')('ct-remote-area');
  const meal       = _w('getAmountVal')('ct-meal');
  const research   = _w('getAmountVal')('ct-research');
  const site_ct    = _w('getAmountVal')('ct-site')||0;
  const skill_ct   = _w('getAmountVal')('ct-skill')||0;
  const license_ct = _w('getAmountVal')('ct-license')||0;
  const comm_ct    = _w('getAmountVal')('ct-communication')||0;
  const fitness_ct = _w('getAmountVal')('ct-fitness')||0;
  const selfDev_ct = _w('getAmountVal')('ct-self-dev')||0;
  const book_ct    = _w('getAmountVal')('ct-book')||0;
  const overseas_ct= _w('getAmountVal')('ct-overseas')||0;
  const regularBonus_ct = _w('getAmountVal')('ct-regular-bonus')||0;

  if(isDaily){
    // ── 일용직: 일 약정일급 + 일일 기준 수당 합산 ──
    const dailyWage  = _w('getAmountVal')('ct-daily-wage');
  ...
  const annualSal = _w('getAmountVal')('ct-annual-sal'); // 정규직: 연봉 / 계약직: 월약정급여
...
export function _getContractHourlyWage(){
  return _w('getAmountVal')('ct-hourly-input') || 0;
}
export function _calcFixedOtFromHours(){
...
export function _calcFixedOtFromPay(){
  const hw  = _getContractHourlyWage();
  const pay = _w('getAmountVal')('ct-fixed-ot-pay');
  const hEl = document.getElementById('ct-fixed-ot-hours');
  if(hw > 0 && pay > 0 && hEl) hEl.value = Math.round(pay / hw / 1.5 * 10) / 10;
}
export function _calcFixedNightFromHours(){
  const hw = _getContractHourlyWage();
...
export function _calcFixedNightFromPay(){
  const hw  = _getContractHourlyWage();
  const pay = _w('getAmountVal')('ct-fixed-night-pay');
  const hEl = document.getElementById('ct-fixed-night-hours');
  if(hw > 0 && pay > 0 && hEl) hEl.value = Math.round(pay / hw / 0.5 * 10) / 10;
}
export function _calcFixedHolFromHours(){
  const hw = _getContractHourlyWage();
...
export function _calcFixedHolFromPay(){
  const hw  = _getContractHourlyWage();
  const pay = _w('getAmountVal')('ct-fixed-hol-pay');
...
// ══ window 등록 (레거시 호환) ══
window.toggleEmExpire = toggleEmExpire;
window._checkFixedTermDuration = _checkFixedTermDuration;
window.calcAnnualLeaveDays = calcAnnualLeaveDays;
window._autoFillCTPeriod = _autoFillCTPeriod;
window._ctPeriodCompose = _ctPeriodCompose;
window._ctPeriodRestore = _ctPeriodRestore;
window.autoFillAnnualLeave = autoFillAnnualLeave;
window.toggleAnnualSal = toggleAnnualSal;
window.toggleProbation = toggleProbation;
window.onProbationBasisChange = onProbationBasisChange;
window.getProbationBase = getProbationBase;
window.calcProbationFromPct = calcProbationFromPct;
window.calcProbationFromAmt = calcProbationFromAmt;
window._checkProbMinWageWarning = _checkProbMinWageWarning;
window._checkMinWageWarning = _checkMinWageWarning;
window._checkRegisterBtnState = _checkRegisterBtnState;
window._checkAmendBtnState = _checkAmendBtnState;
window.syncProbation = syncProbation;
window._bulkWeekdayToggle = _bulkWeekdayToggle;
window._bulkDayToggle = _bulkDayToggle;
window.applyBulkSchedule = applyBulkSchedule;
window._shiftGroupHTML = _shiftGroupHTML;
window._deactivateShift = _deactivateShift;
window._brkSlotsHTML2 = _brkSlotsHTML2;
window._addShift = _addShift;
window._removeShift = _removeShift;
window._addBrkSlot2 = _addBrkSlot2;
window._removeBrkSlot2 = _removeBrkSlot2;
window._getBrkSlots2 = _getBrkSlots2;
window._getBrkSlots = _getBrkSlots;
window._addBrkSlot = _addBrkSlot;
window._removeBrkSlot = _removeBrkSlot;
window._setBrkSlots = _setBrkSlots;
window.initScheduleTable = initScheduleTable;
window.timeToMins = timeToMins;
window.calcWorkHours = calcWorkHours;
window.getScheduleJSON = getScheduleJSON;
window.setScheduleFromJSON = setScheduleFromJSON;
window.setScheduleFromLegacy = setScheduleFromLegacy;
window.initBreakSelects = initBreakSelects;
window.getBreakMins = getBreakMins;
window.setBreakMins = setBreakMins;
window.toggleCtEndDate = toggleCtEndDate;
window.setCTPayType = setCTPayType;
window._getCTPayTypeVal = _getCTPayTypeVal;
window._isFixedAllow = _isFixedAllow;
window._resetCTPayTypes = _resetCTPayTypes;
window.applyCTAllowanceConfig = applyCTAllowanceConfig;
window.onCtStartChange = onCtStartChange;
window.onCtCompanyChange = onCtCompanyChange;
window._setCtPayDayDefault = _setCtPayDayDefault;
window._forceShowNonZeroCTRows = _forceShowNonZeroCTRows;
window._calcMonthlyStdHours = _calcMonthlyStdHours;
window.calcContractSalary = calcContractSalary;
window._getContractHourlyWage = _getContractHourlyWage;
window._calcFixedOtFromHours = _calcFixedOtFromHours;
window._calcFixedOtFromPay = _calcFixedOtFromPay;
window._calcFixedNightFromHours = _calcFixedNightFromHours;
window._calcFixedNightFromPay = _calcFixedNightFromPay;
window._calcFixedHolFromHours = _calcFixedHolFromHours;
window._calcFixedHolFromPay = _calcFixedHolFromPay;
