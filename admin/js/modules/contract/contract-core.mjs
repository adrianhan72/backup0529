/**
 * contract/contract-core.mjs — Phase 6: 계약 관리 코어
...
﻿function _renderContAlertCards(){
  ...
  // 현재 고객사 계약 전체 분류
  const companyContracts = getContracts().filter(c => c.company_id === currentContCompanyId);
  ...
  const CARD_CONFIG = [
    {
      ...
      row: (c) => {
        ...
        return `<td style="font-weight:700;color:#1f2937;">${_w('getEmpName')(c.employee_id)}</td>
          <td><span class="badge ${catBadge}" style="font-size:11px;">${_w('contractTypeLabel')(empCat)}</span></td>
          ...
          <td style="white-space:nowrap;">
            <button onclick="_w('viewContract')('${c.id}')" class="btn btn-sm btn-indigo"><i class="fas fa-search"></i> 조회</button>
          </td>`;
      }
    },
    {
...
export function _toggleContAlertCard(headEl){
...
export function renderContracts(){
  ...
  let f=getContracts().filter(c=>{
    ...
    // 직원명 검색
    if(q&&!_w('getEmpName')(c.employee_id).toLowerCase().includes(q)) return false;
    // 고용형태 필터
    if(filterEmpCat){
      const emp=getEmployees().find(e=>e.id===c.employee_id);
      ...
      if(filterEmpCat === 'representative'){
        const co2 = (getCompanies()||[]).find(x => x.id === c.company_id);
        let reps2 = [];
  ...
  tb.innerHTML=paged.map(c=>{
    ...
    const empCat=typeof normalizeContractType === 'function' ? _w('normalizeContractType')(empCatRaw) : empCatRaw;
    const catBadge=CAT_BADGE_CLS[empCat]||'badge-gray';
    ...
    if(empName){
      // 대표자 확인
      const co = (getCompanies()||[]).find(x => x.id === c.company_id);
      if(co){
        let reps = [];
    ...
    return `<tr>
      <td style="font-weight:600">${_w('getEmpName')(c.employee_id)}</td>
      <td><span class="badge ${catBadge}">${_w('contractTypeLabel')(empCat)}</span>${specialBadge}</td>
      ...
      <td style="white-space:nowrap;">
        <button class="btn btn-sm btn-indigo" onclick="_w('viewContract')('${c.id}')"><i class="fas fa-search"></i> 조회</button>
        ${c.is_draft
          ? `<button class="btn btn-sm" disabled title="임시저장 상태에서는 출력할 수 없습니다"><i class="fas fa-file-contract"></i> 계약서</button>`
          : `<button class="btn btn-sm btn-indigo" onclick="_w('openContractPrintModal')('${c.id}')"><i class="fas fa-file-contract"></i> 계약서</button>`
        }
...
export function setContPage(p){pages.cont=p;_w('renderContracts')()}
export function filterContracts(){pages.cont=1;_w('renderContracts')()}

// ── 계약서 고유 ID 클립보드 복사 ──
export function _copyContractId(id){
  if(!id) return;
  navigator.clipboard.writeText(id).then(()=>{
    if(typeof toast === 'function') _w('toast')(`계약서 고유 ID가 복사되었습니다. (${id})`);
  }).catch(()=>{
    if(typeof toast === 'function') _w('toast')('복사에 실패했습니다.', 'error');
  });
}
// ── 계약 모달 유효성 검사 오류 초기화 ──
export function _ctClearErrors(){
...
// ── 계약 시작일 ↔ 입사일 연동 ──
export function _onCtHireChange(){
...
export function _validateCtStartVsHire(startId, hireId, hintId){
...
export function openContractModal(id=null, preCompanyId=null){
  ...
  if(isNew){
    ...
    { const _co = preCompanyId ? (getCompanies()||[]).find(x=>x.id===preCompanyId) : null;
    ...
    { const _newCo = preCompanyId ? (getCompanies()||[]).find(x=>x.id===preCompanyId) : null;
    ...
    if(c){
      ...
      if(emp){
        ...
        const _empCatKorean = _w('contractTypeLabel')(_empCatRaw) || '-';
      ...
      { const _co = c.company_id ? (getCompanies()||[]).find(x=>x.id===c.company_id) : null;
        const _nameEl = document.getElementById('ct-company-name'); if(_nameEl) _nameEl.textContent = _co?.company_name || ''; }
...
export function _onEditCategoryChange() {
...
export function _resetWageInputs(){
  // 연봉·월약정급여·기본급·일급
...
export function _setEditNameCategoryLock(lock, lockCat = lock) {
 ...
 */
export function _checkRepSelf(nameInputId, repRowId, repChkId, altNamesId){
  ...
  if(isRep){
    ...
    if(altEl){
      ...
      const existingNames = getEmployees()
...
export function _repUseAltName(inputId, rowId, altName){
...
export function checkCtDuplicateName(){
...
// ── 수정 모드(이어쓰기) 이름 중복 검사 ──
export function checkCtEditDuplicateName(){
  ...
  const c = editId.contract ? getContracts().find(x => x.id === editId.contract) : null;
...
export function _renderNameDuplicateAlert(name, coId, selfEmpId, inputId, alertPrefix){
  ...
  const allDupes = getEmployees().filter(e => e.company_id===coId && e.name===name && e.id!==selfEmpId);
  const dupes = allDupes.filter(emp => {
    return (getContracts()||[]).some(c =>
  ...
  const existingNames = getEmployees().filter(e=>e.company_id===coId && new RegExp(`^${escaped}(\\d+)?$`).test(e.name)).map(e=>e.name);
  const suggestions=[];
  ...
  const dupesWithStatus = dupes.map(emp => {
    const activeCt = (getContracts()||[]).find(c =>
      c.employee_id === emp.id && VALID_FOR_NAME.has(c.status)
    );
    const latestCt = (getContracts()||[])
...
export function _ctDupRenew(contractId){
...
export function _ctDupEdit(contractId){
  _hideAllNameDupAlerts();
  _w('closeModal')('contract-modal');
...
export function _ctDupRecontract(contractId){
  _hideAllNameDupAlerts();
  _w('closeModal')('contract-modal');
  setTimeout(() => {
    const src = (getContracts()||[]).find(c => c.id === contractId);
    if(src && typeof openRecontractModal === 'function') openRecontractModal(src);
  }, 200);
}

export function _hideAllNameDupAlerts(){
 ...
 */
export function _validateEmpNoUniqueness(empNo, companyId, selfEmpId, newContractStart, newContractType) {
  if(!empNo || !companyId) return { ok: true, type: 'ok', msg: '' };

  // ── 같은 회사 내 동일 사원번호를 가진 직원 탐색 (해지·만료·파기·취소·임시저장 포함 모든 계약) ──
  const sameNoEmps = getEmployees().filter(e =>
 ...
 * @returns {{ ok:boolean, type:string, msg:string }}
 */
export function _checkEmpNoContinuity(empNo, companyId, selfEmpId, newStart, newType) {
  ...
  const myContracts = getContracts().filter(c =>
...
export function checkCtEmpNoUniqueness(){
...
export function checkCtEditEmpNoUniqueness(){
...
export function _showEmpNoAlert(alertEl, msg, type){
  const isOk = (type === 'ok');
  alertEl.style.display = 'block';
...
export function getEarliestContractStart(employeeId){
  if(!employeeId || !Array.isArray(getContracts())) return '';
  const dates = getContracts()
...
// ─── 계약 상태 표시 계산 ───
export function calcContractStatusDisplay(c, today){
...
export function viewContract(id){
  ...
  if(c){
    const emp2 = getEmployees().find(e => e.id === c.employee_id);
  ...
  ['ct-btn-recontract','ct-btn-recontract2'].forEach(bid=>{
    ...
    if(showRecontract && c?.employee_id){
      // 동일인 활성 계약 존재 시 숨김
      const hasActive = getContracts().some(x =>
      ...
      // 가장 최근 만료/해지 계약인지 확인
      if(showRecontract){
        const latestTerminated = getContracts()
          .filter(x =>
            x.employee_id === c.employee_id &&
...
export function doContractAmend(){
  const modalEl = document.querySelector('#contract-modal .modal');
  if(!modalEl) return;

  // 계약예정: 시작일 당일부터 수정 불가
  const _amendC = getContracts().find(x => x.id === editId.contract);
  if(_amendC && _amendC.status===CONTRACT_STATUS.PENDING){
    const _today = new Date().toISOString().slice(0,10);
    if(_amendC.contract_start && _today >= _amendC.contract_start){
      _w('toast')(`계약 시작일(${_amendC.contract_start}) 이후에는 예정 계약을 수정할 수 없습니다.`, 'error');
      return;
    }
  }
...
// 수정 취소: _w('viewContract')()로 복원
export function cancelContractAmend(){
...
export async function openAmendPreview(){
  if(!editId.contract) return;
  // 유효성 재확인
  if(_ctValidate()) return;

  const origId = editId.contract;
  const origC  = getContracts().find(x => x.id === origId);
  if(!origC){ _w('toast')('원본 계약 정보를 찾을 수 없습니다.', 'error'); return; }

  // 계약서 HTML 생성
  let html;
  try { html = generateContractHTML(); }
  catch(e){ console.error('[openAmendPreview] generateContractHTML 오류:', e); _w('toast')('계약서 미리보기 생성 중 오류가 발생했습니다.', 'error'); return; }
  ...
  if(!_isPendingAmend){
    try {
      ...
      const origIdx = getContracts().findIndex(x => x.id === origId);
      if(origIdx !== -1){
        getContracts()[origIdx].status             = CONTRACT_STATUS.VOIDED;
        getContracts()[origIdx].is_voided_by_amend = true;
        getContracts()[origIdx].voided_at          = nowISO;
      }
    } catch(e){
      console.error('[원본 파기 처리 오류]', e);
      _w('toast')('원본 계약서 파기 처리에 실패했습니다.', 'error');
      return;
    }
  }
  ...
  const base_   = isDailyA ? 0 : _w('getAmountVal')('ct-base');
  const dWage_  = isDailyA ? _w('getAmountVal')('ct-daily-wage') : 0;
  const annualSalInput_ = _w('getAmountVal')('ct-annual-sal');
  const annual_ = isRegGrp ? annualSalInput_ : 0;
  const wkHol_  = isDailyA ? 0 : Math.round(base_ / days_);  // 단시간 비례: ÷dpw
  const pos_    = _w('getAmountVal')('ct-position');
  const car_    = _w('getAmountVal')('ct-car');
  const meal_   = _w('getAmountVal')('ct-meal');
  const res_    = _w('getAmountVal')('ct-research');
  const other_  = _w('getAmountVal')('ct-other')||0;
  const regBonus_= _w('getAmountVal')('ct-regular-bonus')||0;
  const site_   = _w('getAmountVal')('ct-site')||0;
  const skill_  = _w('getAmountVal')('ct-skill')||0;
  const lic_    = _w('getAmountVal')('ct-license')||0;
  const comm_   = _w('getAmountVal')('ct-communication')||0;
  const fit_    = _w('getAmountVal')('ct-fitness')||0;
  const sdev_   = _w('getAmountVal')('ct-self-dev')||0;
  const book_   = _w('getAmountVal')('ct-book')||0;
  const ovseas_ = _w('getAmountVal')('ct-overseas')||0;
  ...
  const commonFields = {
    ...
    schedule_json: (() => {
      const _newSch = _w('getScheduleJSON')();
    ...
    remote_area_allowance: _w('getAmountVal')('ct-remote-area'), remote_area_pay_type:'fixed',
  ...
  if(_isPendingAmend){
    ...
    try {
      ...
      const idx = getContracts().findIndex(x => x.id === origId);
      if(idx !== -1) getContracts()[idx] = { ...allContracts[idx], ...putBody };
    } catch(e){
      console.error('[계약예정 수정 저장 오류]', e);
      _w('toast')('계약 수정 저장에 실패했습니다.', 'error');
      return;
    }
    // 계약예정 수정은 모달 없이 즉시 완료
    _w('closeModal')('contract-modal');
    await _w('loadContracts')(); await _w('loadEmployees')();
    _w('renderContracts')(); _w('renderDashboard')();
    _w('toast')('예정 계약이 수정되었습니다. ✔', 'success');
    return;
  }
  ...
  try {
    ...
    getContracts().push({ ...newBody, id: newContractId });
  } catch(e){
    console.error('[재발행 계약서 저장 오류]', e);
    _w('toast')('재발행 계약서 저장에 실패했습니다.', 'error');
    return;
  }

  // ④ 발송 이력 기록
  const _emp = getEmployees().find(e => e.id === empId) || {};
 ...
 */
export function _openAmendResultModal(newContractId){
  // 기존 배너 제거
  ...
  _w('openContractPrintModal')(newContractId);
}

// ──────────────────────────────────────────────────────────────────────────────
// finalAmendContract() — 하위호환용 stub (더 이상 사용 안 함, openAmendPreview로 통합)
// ──────────────────────────────────────────────────────────────────────────────
export async function finalAmendContract(){
  // 더 이상 사용되지 않습니다. openAmendPreview()에서 즉시 저장 처리합니다.
  _w('toast')('수정 재발행은 수정완료 버튼 클릭 시 자동 저장됩니다.', 'warning');
}


// ══ window 등록 (레거시 호환) ══
window._toggleContAlertCard = _toggleContAlertCard;
window.renderContracts = renderContracts;
window.setContPage = setContPage;
window.filterContracts = filterContracts;
window._copyContractId = _copyContractId;
window._ctClearErrors = _ctClearErrors;
window._onCtHireChange = _onCtHireChange;
window._validateCtStartVsHire = _validateCtStartVsHire;
window.openContractModal = openContractModal;
window._ctClearFieldError = _ctClearFieldError;
window.onCtEmployeeChange = onCtEmployeeChange;
window._onEditCategoryChange = _onEditCategoryChange;
window._resetWageInputs = _resetWageInputs;
window._setEditNameCategoryLock = _setEditNameCategoryLock;
window._checkRepSelf = _checkRepSelf;
window._repUseAltName = _repUseAltName;
window.checkCtDuplicateName = checkCtDuplicateName;
window.checkCtEditDuplicateName = checkCtEditDuplicateName;
window._renderNameDuplicateAlert = _renderNameDuplicateAlert;
window._ctDupRenew = _ctDupRenew;
window._ctDupEdit = _ctDupEdit;
window._ctDupRecontract = _ctDupRecontract;
window._hideAllNameDupAlerts = _hideAllNameDupAlerts;
window._validateEmpNoUniqueness = _validateEmpNoUniqueness;
window._checkEmpNoContinuity = _checkEmpNoContinuity;
window.checkCtEmpNoUniqueness = checkCtEmpNoUniqueness;
window.checkCtEditEmpNoUniqueness = checkCtEditEmpNoUniqueness;
window._showEmpNoAlert = _showEmpNoAlert;
window.getEarliestContractStart = getEarliestContractStart;
window.calcContractStatusDisplay = calcContractStatusDisplay;
window.viewContract = viewContract;
window.doContractAmend = doContractAmend;
window.cancelContractAmend = cancelContractAmend;
window.openAmendPreview = openAmendPreview;
window._openAmendResultModal = _openAmendResultModal;
window.finalAmendContract = finalAmendContract;
