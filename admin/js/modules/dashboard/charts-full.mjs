/**
 * dashboard/charts-full.mjs — Phase 6: 대시보드 차트
...
//   선택된 고객사·직원과 무관하게 getPayrolls() 내 is_draft:true 전건을 표시.
//   건수 0이면 배너 숨김.
// ==============================================================================
export function renderPIAllDraftBanner(){
  ...
  const _heavyReady = typeof window._heavyDataReady !== 'undefined' && window._heavyDataReady;
  if(!_heavyReady){
...
export function togglePIAllDraftBanner(headerEl){
...
export function renderDashboard(){
  ...
  const activeCompanyCount=getCompanies().filter(c=>isCompanyActive(c)).length;
...
export function renderDashBillingCards(){
  // 급여·청구 데이터 미준비 시 로딩 표시
  if(!window._heavyDataReady){
  ...
  // 이번달 실제 청구 건
  const thisMonthBillings = getPayrolls().filter(b =>
  ...
  const overdueBillings = getPayrolls().filter(b => {
    if(b.payment_status ===PAYMENT_STATUS.PAID) return false;
    const rem = (b.total_amount || 0) - (b.partial_paid_amount || 0);
    if(rem <= 0) return false;
...
export function renderCompanyTrendChart(){
  ...
  getCompanies().forEach(c => {
    ...
    const execNamesD = (window.allExecutives||[]).filter(e => e.company_id === c.id).map(e => e.name).filter(Boolean);
    const relNamesD  = (window.allRelatedParties||[]).filter(r => r.company_id === c.id).map(r => r.name).filter(Boolean);
    const excludedNames = new Set([...repNamesD, ...execNamesD, ...relNamesD]);
    const repEmpIds = new Set(
      (getEmployees()||[]).filter(e => e.company_id === c.id && (e.is_representative || excludedNames.has(e.name))).map(e => e.id)
    );
    const empCount = getContracts().filter(ct =>
...
export function renderEmployeeTrendChart(){
  const rangeEl = document.getElementById('dash-chart-range');
  const months = rangeEl ? parseInt(rangeEl.value) : 12;
  ...
  const activeCompanyIds = new Set(
    getCompanies().filter(c => isCompanyActive(c)).map(c => c.id)
  );
...
export function renderBillingTrendChart(){
  if(!window._heavyDataReady) return; // 급여·청구 데이터 미준비 시 skip
  ...
  for(let i = months - 1; i >= 0; i--){
    ...
    // 해당 월 청구 건
    const monthBillings = getPayrolls().filter(b =>
      Number(b.billing_year) === yr && Number(b.billing_month) === mo
    );
    ...
    const totalUnpaid = getPayrolls().reduce((s, b) => {
    ...
    const totalLoss = getPayrolls().reduce((s, b) => {
...
export function goCompaniesWithFilter(status){
...
export function _buildDashCompanyRow(c){
  const now = new Date();
...
export function renderDashboardCompanies(){
  // 이용중 고객사만 표시
  const searchInput=document.getElementById('dash-company-search');
  const searchTerm=searchInput?searchInput.value.toLowerCase().trim():'';

  let companies=getCompanies().filter(c=>isCompanyActive(c));
...
// ══ window 등록 (레거시 호환) ══
window.renderPIAllDraftBanner = renderPIAllDraftBanner;
window.togglePIAllDraftBanner = togglePIAllDraftBanner;
window.renderDashboard = renderDashboard;
// [사용료 숨김] window.renderDashBillingCards = renderDashBillingCards;
window.renderCompanyTrendChart = renderCompanyTrendChart;
window.renderEmployeeTrendChart = renderEmployeeTrendChart;
// [사용료 숨김] window.renderBillingTrendChart = renderBillingTrendChart;
window.goCompaniesWithFilter = goCompaniesWithFilter;
window._buildDashCompanyRow = _buildDashCompanyRow;
window.renderDashboardCompanies = renderDashboardCompanies;
