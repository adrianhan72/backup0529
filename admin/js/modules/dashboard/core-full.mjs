/**
 * dashboard/core-full.mjs — Phase 6: 대시보드 코어
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
export function renderDashExpiryBanner(){
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
export function renderDashRegularBanner(){
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
export function _getProbationNoticeTargets(){
...
// ─────────────────────────────────────────────────────────────────
export function _getProbationAllTargets(){
  const today = new Date();
  today.setHours(0,0,0,0);
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
export function renderDashProbationBanner(){
...
export function selectProbMgmtCompanyFromDash(coId, coName, contractId){
...
window._probMgmtSelectedCoId = null;
window._probMgmtSelectedCoName = '';

export function renderProbMgmtCompanyList(){
...
export function selectProbMgmtCompany(coId, coName){
...
export function clearProbMgmtCompany(){
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
export function renderProbationMgmtTable(){
...
/* ── 모달 열기 ── */
export function openProbMgmtModal(contractId){
  ...
  if(c.probmgmt_action === 'confirm'){
    ...
    const pendingContract = getContracts().find(x =>
...
/* ── 옵션 선택 (카드 확장) ── */
export function selectProbMgmtOpt(key){
...
/* ── 수습 연장 미리보기 ── */
export function updateProbExtendPreview(){
...
/* ── 모달 닫기 ── */
export function closeProbMgmtModal(){
...
/* ── 처리결과 PATCH 공통 헬퍼 ── */
export async function _patchProbmgmtAction(contractId, action){
  ...
  const idx = getContracts().findIndex(c => c.id === contractId);
  if(idx > -1){
    getContracts()[idx].probmgmt_action    = action;
    getContracts()[idx].probmgmt_action_at = nowISO;
  }
}
...
/* ── 채용 확정 ── */
export async function execProbConfirm(){
  const targets = _getProbationAllTargets();
  const t = targets.find(x => x.contract.id === _probMgmtCurrentId);
  if(!t) return;
  const c = t.contract;
...
/* ── 채용 확정 처리취소 ── */
export async function execProbConfirmRevoke(){
  const targets = _getProbationAllTargets();
  const t = targets.find(x => x.contract.id === _probMgmtCurrentId);
  if(!t) return;
  const c = t.contract;
  ...
  try {
    ...
    if(pendingContract){
      ...
      const pi = getContracts().findIndex(x => x.id === pendingContract.id);
      if(pi > -1){
        getContracts()[pi].status             = CONTRACT_STATUS.VOIDED;
        getContracts()[pi].is_voided_by_amend = true;
        getContracts()[pi].voided_at          = nowISO;
      }
    }
    ...
    const ci = getContracts().findIndex(x => x.id === c.id);
    if(ci > -1){
      getContracts()[ci].probmgmt_action    = null;
      getContracts()[ci].probmgmt_action_at = null;
    }
...
/* ── 채용 취소 ── */
export async function execProbCancel(){
  const targets = _getProbationAllTargets();
  const t = targets.find(x => x.contract.id === _probMgmtCurrentId);
  if(!t) return;
  ...
  try {
    ...
    const idx = getContracts().findIndex(c => c.id === t.contract.id);
    if(idx > -1) getContracts()[idx].status = CONTRACT_STATUS.EXPIRED;
...
/* ── 조기 해고 ── */
export async function execProbDismiss(){
  const targets = _getProbationAllTargets();
  const t = targets.find(x => x.contract.id === _probMgmtCurrentId);
  if(!t) return;
  ...
  try {
    ...
    const idx = getContracts().findIndex(c => c.id === t.contract.id);
    if(idx > -1) getContracts()[idx].status = CONTRACT_STATUS.TERMINATED;
...
/* ── 수습 연장 ── */
export async function execProbExtend(){
  const targets = _getProbationAllTargets();
  const t = targets.find(x => x.contract.id === _probMgmtCurrentId);
  if(!t) return;
  ...
  try {
    ...
    const origIdx = getContracts().findIndex(c => c.id === origC.id);
    if(origIdx !== -1){
...
/* ─────────────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────────────── */
window.window.allSeveranceNotices = [];   // severance_paid 미읽음 캐시

export async function loadSeveranceNotices(){
  try {
...
export function renderDashSeveranceBanner(){
  const sec = document.getElementById('dash-severance-banner');
  if(!sec) return;
  if(!window.window.allSeveranceNotices.length){ sec.style.display = 'none'; sec.innerHTML = ''; return; }

  // 고객사별 집계
  const byCoMap = {};
  window.allSeveranceNotices.forEach(n => {
  ...
  const total = window.allSeveranceNotices.length;
...
export function renderDraftAlerts(){
  ...
  const draftCompanies = getCompanies().filter(c => !!c.is_draft);
  const draftContracts = getContracts().filter(c => !!c.is_draft);
  // 급여 임시저장: is_draft:true 레코드 (직원·연월 기준 최신 1건만 표시)
  const draftPayrolls  = (getPayrolls() || []).filter(p => !!p.is_draft);
  ...
  const payrollRows = sortedDraftPayrolls.map(p => {
    ...
    const co     = getCompanies().find(x => x.id === p.company_id);
    const empName= emp ? emp.name : '(직원 미지정)';
    const coName = co  ? (co.company_name || '-') : '-';
    const yrMo   = (p.pay_year && p.pay_month)
      ? `${p.pay_year}년 ${p.pay_month}월`
      : '';
    const empCat = typeof contractTypeLabel === 'function' ? _w('contractTypeLabel')(emp?.employment_category) : (emp?.employment_category || '');
    // 급여일: 근로계약서 pay_day > 급여레코드 pay_date > 고객사 pay_day
    const ct = (getContracts()||[]).find(c => c.employee_id === p.employee_id && c.company_id === p.company_id && !c.is_draft && (c.status==='active'||c.status==='활성'));
...
/* =================================================================
   ...
   ================================================================= */
export function _renderContractsBanners(){
  // ── 공통 헬퍼 ──
  function fmtTime(ts){
    if(!ts) return '';
  ...
  (function(){
    ...
    if(!sec) return;
    const drafts = getContracts().filter(c => !!c.is_draft);
...
export function renderSignedAlerts(){
...
export function renderConsentAlerts(){
  // [사용안함] 서류미비 계약도 유효 계약으로 처리하므로 알림 카드 제거
...
// ─── 대시보드 알림 카드 아코디언 토글 ───
export function toggleDashAccordion(bodyId, toggleBtn, event){
...
export function goDraftCompany(id){
...
export function goDraftContract(id){
...
export function goDraftPayroll(draftId){
  const p = (getPayrolls()||[]).find(x => x.id === draftId);
  if(!p){ _w('toast')('임시저장 데이터를 찾을 수 없습니다.', 'error'); return; }
...
// ══ window 등록 (레거시 호환) ══
window.renderDashExpiryBanner = renderDashExpiryBanner;
window.renderDashRegularBanner = renderDashRegularBanner;
window._getProbationNoticeTargets = _getProbationNoticeTargets;
window._getProbationAllTargets = _getProbationAllTargets;
window.renderDashProbationBanner = renderDashProbationBanner;
window.selectProbMgmtCompanyFromDash = selectProbMgmtCompanyFromDash;
window.renderProbMgmtCompanyList = renderProbMgmtCompanyList;
window.selectProbMgmtCompany = selectProbMgmtCompany;
window.clearProbMgmtCompany = clearProbMgmtCompany;
window.renderProbationMgmtTable = renderProbationMgmtTable;
window.openProbMgmtModal = openProbMgmtModal;
window.selectProbMgmtOpt = selectProbMgmtOpt;
window.updateProbExtendPreview = updateProbExtendPreview;
window.closeProbMgmtModal = closeProbMgmtModal;
window._patchProbmgmtAction = _patchProbmgmtAction;
window.execProbConfirm = execProbConfirm;
window.execProbConfirmRevoke = execProbConfirmRevoke;
window.execProbCancel = execProbCancel;
window.execProbDismiss = execProbDismiss;
window.execProbExtend = execProbExtend;
window.loadSeveranceNotices = loadSeveranceNotices;
window.renderDashSeveranceBanner = renderDashSeveranceBanner;
window.renderDraftAlerts = renderDraftAlerts;
window._renderContractsBanners = _renderContractsBanners;
window.renderSignedAlerts = renderSignedAlerts;
window.renderConsentAlerts = renderConsentAlerts;
window.toggleDashAccordion = toggleDashAccordion;
window.goDraftCompany = goDraftCompany;
window.goDraftContract = goDraftContract;
window.goDraftPayroll = goDraftPayroll;
