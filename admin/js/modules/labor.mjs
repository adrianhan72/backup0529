...
export function renderLsCompanyList(){
  ...
  container.innerHTML = companies.map(c => {
    ...
    const activeCtEmpIds = new Set(
      getContracts().filter(ct => ct.company_id === c.id && (ct.status === CONTRACT_STATUS.ACTIVE || ct.status === CONTRACT_STATUS.DOCS_INCOMPLETE)).map(ct => ct.employee_id)
    );
    const empCnt = activeCtEmpIds.size;
    const contractCnt = getContracts().filter(ct => ct.company_id === c.id && (ct.status === CONTRACT_STATUS.ACTIVE || ct.status === CONTRACT_STATUS.DOCS_INCOMPLETE)).length;
...
// 연월 셀렉트 초기화
export function initLsYearMonth(){
  const now = new Date();
...
// 고객사 선택
export function selectLsCompany(companyId, companyName){
...
// 고객사 선택 해제
export function clearLsCompanySelect(){
...
export function renderLsPieChart(){
  const selCo = window.currentLsCompanyId;
  if(!selCo) return;
  ...
  const pays = getPayrolls().filter(p => !p.is_draft && p.company_id===selCo && p.pay_year==yr && p.pay_month==mo);
...
export function switchLsDistTab(tab){
...
export function renderLsDistChart(){
  const selCo = window.currentLsCompanyId || window.currentGlobalCompanyId;
  if(!selCo) return;
  const yr = parseInt(document.getElementById('ls-year')?.value) || new Date().getFullYear();
  const mo = parseInt(document.getElementById('ls-month')?.value) || new Date().getMonth()+1;
  const pays = getPayrolls().filter(p => !p.is_draft && p.company_id===selCo && p.pay_year==yr && p.pay_month==mo);
  ...
  if(currentLsDistTab === 'emp'){
    groups = pays
      .map(p => ({
        label: _w('getEmpName')(p.employee_id),
        gross: p.gross_pay||0,
        net:   p.net_pay||0,
      }))
...
export function renderLsYearTrendChart(){
  const selCo = window.currentLsCompanyId || window.currentGlobalCompanyId;
  if(!selCo) return;
  ...
  const monthData = months.map(({y,m}) => {
    const pays = getPayrolls().filter(p => !p.is_draft && p.company_id === selCo && p.pay_year == y && p.pay_month == m);
...
export function renderLaborStatus(){
  const yr = parseInt(document.getElementById('ls-year')?.value) || new Date().getFullYear();
  const mo = parseInt(document.getElementById('ls-month')?.value) || (new Date().getMonth() + 1);
  ...
  // ── 통계 계산 ──
  const targetContracts = getContracts().filter(c => c.company_id === selCo);
  ...
  const targetEmps = getEmployees().filter(e => e.company_id === selCo && activeEmpIds.has(e.id));
  ...
  const catCount = cat => targetEmps.filter(e => _w('normalizeContractType')(e.employment_category) === cat).length;
  ...
  const thisPays = getPayrolls().filter(p => !p.is_draft && p.company_id === selCo && p.pay_year == yr && p.pay_month == mo);
  ...
  filteredContracts = [...filteredContracts].sort((a, b) => {
    ...
    return _w('getEmpName')(a.employee_id).localeCompare(_w('getEmpName')(b.employee_id), 'ko');
  });
  ...
  if(filteredContracts.length === 0){
    ...
    ctTbody.innerHTML = filteredContracts.map(c => {
      ...
      const catBadge = _w('empCatBadge')(empCat);
      ...
      return `<tr>
        ...
        <td><span class="badge ${catBadge}" style="font-size:10.5px;">${_w('contractTypeLabel')(empCat)}</span></td>
  ...
  if(thisPays.length === 0){
    ...
    const sortedPays = [...thisPays].sort((a,b) => _w('getEmpName')(a.employee_id).localeCompare(_w('getEmpName')(b.employee_id),'ko'));
...
// ══ window 등록 (레거시 호환) ══
window.renderLsCompanyList = renderLsCompanyList;
window.initLsYearMonth = initLsYearMonth;
window.selectLsCompany = selectLsCompany;
window.clearLsCompanySelect = clearLsCompanySelect;
window.renderLsPieChart = renderLsPieChart;
window.switchLsDistTab = switchLsDistTab;
window.renderLsDistChart = renderLsDistChart;
window.renderLsYearTrendChart = renderLsYearTrendChart;
window.renderLaborStatus = renderLaborStatus;
