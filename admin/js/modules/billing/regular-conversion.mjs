/**
 * billing/regular-conversion.mjs — Phase 6: 완전 변환
 ...
 */
export function _calcRcFullList(){
 ...
 * 정규직 전환 관리 페이지 초기화
 */
export async function initRcPage(){
  _rcTab = 'target';
  _rcTargetPage  = 1;
  _rcHistoryPage = 1;
 ...
 * RC 탭 전환
 */
export function rcSwitchTab(tab){
  _rcTab = tab;
 ...
 */
export function renderRcTargetList(){
  ...
  tbody.innerHTML = pageData.map(x => {
    const catText   = _w('contractTypeLabel')(x.activeContract?.contract_type) || '계약직';
 ...
 */
export async function rcLoadHistory(force=false){
 ...
 * 발송 이력 테이블 렌더링
 */
export function renderRcHistory(){
 ...
 */
export function _rcFillCompanyFilter(selId){
 ...
 * 메시지 예시 샘플 셀렉트 채우기
 */
export function _rcFillTemplateSampleSel(){
 ...
 */
export function renderRcTemplate(){
  ...
  const coName     = item?.company        || '(주)예시기업';
    const coRep      = item ? _w('getCompanyRepName')(getCompanies().find(c=>c.id===item.companyId)) : '대표자';
 ...
 */
export async function rcSendNotice(empId){
 ...
 * RC 페이지 전체 새로고침
 */
export async function rcRefresh(){
  _rcHistoryLoaded = false;
  await rcLoadHistory(true);
  _rcFillCompanyFilter('rc-filter-company');
  renderRcStats();
 ...
 */
export function _rcBuildPagination(total, current, pageVar, renderFn){
  if(total <= 1) return '';
  const btns = [];
...
// ══ window 등록 (레거시 호환) ══
window._calcRcFullList = _calcRcFullList;
window.initRcPage = initRcPage;
window.rcSwitchTab = rcSwitchTab;
window.renderRcStats = renderRcStats;
window.renderRcTargetList = renderRcTargetList;
window.rcLoadHistory = rcLoadHistory;
window.renderRcHistory = renderRcHistory;
window._rcFillCompanyFilter = _rcFillCompanyFilter;
window._rcFillTemplateSampleSel = _rcFillTemplateSampleSel;
window.renderRcTemplate = renderRcTemplate;
window.rcSendNotice = rcSendNotice;
window.rcRefresh = rcRefresh;
window._rcBuildPagination = _rcBuildPagination;
