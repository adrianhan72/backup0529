/**
 * billing/annual-leave.mjs — Phase 6: 완전 변환
 ...
 */
export function calcEmployeeAnnualLeave(emp, contract, company, refYear){
  ...
  // ── 사용 연차 집계 ──
  const usedDays = (getPayrolls() || []).filter(p => {
...
/** 고객사 칩 렌더링 */
export function renderAlCompanyChips(){
  ...
  const active = getCompanies().filter(c =>
    isCompanyActive(c) &&
  ...
  container.innerHTML = active.map(c => {
    const empCnt = getEmployees().filter(e =>
...
/** 고객사 선택 */
export function selectAlCompany(companyId, companyName){
...
/** 고객사 선택 화면으로 돌아가기 */
export function alBackToCompanyList(){
  // _alCompanyId / window.currentGlobalCompanyId 는 유지 → 칩 강조 표시 보존
  document.getElementById('al-company-select-card').style.display = '';
  document.getElementById('al-main-section').style.display        = 'none';
  renderAlCompanyChips();
}
...
export function renderAlTable(){
  ...
  const co      = getCompanies().find(c => c.id === _alCompanyId);
  ...
  tbody.innerHTML = pageData.map(({emp, contract, al, ledger, effectiveUsed, carryover, effectiveRemain, effectivePay}) => {
    ...
    return `<tr>
      ...
      <td><span class="badge ${badgeCls}" style="font-size:11px;">${_w('contractTypeLabel')(cat)}</span></td>
 ...
 * 연차일수 포맷 — 0.25 단위 고려
 */
export function _fmtLeaveDay(v){
  if(v == null || isNaN(v)) return '-';
  const n = parseFloat(v);
  if(n === Math.floor(n)) return `${n}일`;
  // 소수점 최대 2자리
 ...
 */
export function _buildDaysOptions(current, maxVal){
 ...
 */
export async function openLeaveLedger(empId, empName, refYear){
  _ledgerEmpId    = empId;
 ...
 */
export async function _ledgerNavYear(delta){
  _ledgerYear += delta;
  await _loadAndRenderLedger();
}
...
export async function _loadAndRenderLedger(){
  ...
  // 1) window.allLeaveLedgers 캐시에서 먼저 조회
  let rec = window.allLeaveLedgers.find(r =>
  ...
  if(!rec){
    try{
      ...
      if(fresh){
        // 캐시에 없으면 추가
        const idx = window.allLeaveLedgers.findIndex(x => x.id === fresh.id);
        if(idx > -1) window.allLeaveLedgers[idx] = fresh;
        else window.allLeaveLedgers.push(fresh);
        rec = fresh;
      }
    }catch(e){
 ...
 */
export function _renderLedgerMonthTable(existData){
...
export function _onLedgerDaysChange(sel){
...
export function _recalcLedgerSum(){
  // 월별 사용일수 합산
  let usedSum = 0;
...
/** 관리대장 저장 */
export async function saveLeaveLedger(){
  ...
  try{
    ...
    // ── window.allLeaveLedgers 캐시 갱신 (저장 즉시 목록에 반영) ──
    const existIdx = window.allLeaveLedgers.findIndex(r => r.id === saved.id);
    if(existIdx > -1){
      window.allLeaveLedgers[existIdx] = saved;   // PUT: 기존 레코드 교체
    } else {
      window.allLeaveLedgers.push(saved);          // POST: 신규 추가
    }
    ...
    _w('toast')(`${_ledgerEmpName}의 ${_ledgerYear}년 연차 관리대장이 저장되었습니다.`, 'success');

  }catch(e){
    console.error('[LedgerSave]', e);
    _w('toast')('저장 중 오류가 발생했습니다: ' + e.message, 'error');
  } finally {
 ...
 */
export async function _syncLedgerToPayrolls(empId, year, monthData){
  let updated = 0, skipped = 0;

  for(const { month, days } of monthData){
    // 해당 직원·연도·월의 payroll 레코드 조회 (확정 저장 우선, 없으면 임시저장)
    const payroll = (getPayrolls() || []).find(p =>
      p.employee_id === empId &&
...
/** 사용촉진 모달 열기 */
export function openAlPromoModal(empId){
  const emp      = getEmployees().find(e => e.id === empId);
  if(!emp){ _w('toast')('직원 정보를 찾을 수 없습니다.','error'); return; }
  const contract = getContracts().find(c =>
    c.employee_id === empId && (c.status===CONTRACT_STATUS.ACTIVE||c.status===EMP_STATUS.ACTIVE)
  ) || getContracts().filter(c=>c.employee_id===empId)
                   .sort((a,b)=>(b.contract_start||'').localeCompare(a.contract_start||''))[0];
  if(!contract){ _w('toast')('활성 계약 정보를 찾을 수 없습니다.','error'); return; }

  const co      = getCompanies().find(c => c.id === emp.company_id);
  const refYear = parseInt(document.getElementById('al-year-sel')?.value) || new Date().getFullYear();
  const al      = calcEmployeeAnnualLeave(emp, contract, co, refYear);
  if(!al){ _w('toast')('연차 계산 오류가 발생했습니다.','error'); return; }

  _alPromoEmpId = empId;
  _alPromoData  = { emp, contract, co, al, refYear };

  const adminNm = _w('_getAdminUsername')();
  ...
  if(infoEl){
    const rows = [
      ['직원명',              emp.name||'-'],
      ['고객사',              co?.company_name||'-'],
      ['고용형태',            _w('contractTypeLabel')(emp.employment_category||contract.contract_type) || '-'],
  ...
  _w('openModal')('al-promo-modal');
}

/**
 * 근로자용 사용촉진 메시지 본문 (알림톡·이메일 직접 수신)
 */
export function _buildLeavePromoWorkerBody(emp, co, al, refYear, adminName){
 ...
 */
export function _buildLeavePromoCompanyBody(emp, co, al, refYear, adminName, workerMethod){
...
/** 연차 사용 기한 계산 헬퍼 */
export function _calcLeaveEndDate(emp, al, refYear){
  if(al.basis === '입사일 기준'){
 ...
 */
export async function confirmSendLeavePromotion(method){
  if(!_alPromoEmpId || !_alPromoData){ _w('toast')('발송 대상 정보가 없습니다.','error'); return; }
  ...
  if(method ===DISPATCH_METHOD.KAKAO && !(emp.phone||'').trim()){
    _w('toast')(`${emp.name} — 휴대폰 번호가 등록되지 않았습니다.`, 'error'); return;
  }
  if(method ===DISPATCH_METHOD.EMAIL && !(emp.email||'').trim()){
    _w('toast')(`${emp.name} — 이메일이 등록되지 않았습니다.`, 'error'); return;
  }
  ...
  try {
    ...
    _w('toast')(successMsg, 'success');
    _w('closeModal')('al-promo-modal');
...
/** 사용촉진 이력 DB 조회 */
export async function loadLeavePromotionHistory(force=false){
...
/** LP 페이지 초기화 */
export async function initLpPage(){
  _lpPage = 1;
...
/** LP 고객사 필터 옵션 채우기 */
export function _lpFillCompanyFilter(){
...
export function renderLpTable(){
  ...
  tbody.innerHTML = pageData.map(r=>{
    ...
    return `<tr>
      ...
      <td><span class="badge ${cls}" style="font-size:11px;">${_w('contractTypeLabel')(r.contract_type)}</span></td>
      ...
      <td style="font-size:12px;color:#6b7280;">${_w('_resolveAdminName')(r.sent_by)||'-'}</td>
...
/** 발송 이력 렌더링 */
export function renderCenHistory(){
  ...
  tbody.innerHTML = pageData.map((r,idx)=>`
    <tr style="${idx%2?'background:#fafafa':''}" onmouseover="this.style.background='#f0f9ff'" onmouseout="this.style.background='${idx%2?'#fafafa':''}'">
      ...
      <td><span class="badge ${CAT_BADGE_CLS[r.contract_type]||'badge-gray'}" style="font-size:11px;">${_w('contractTypeLabel')(r.contract_type)}</span></td>
...
// ══ window 등록 (레거시 호환) ══
window.calcEmployeeAnnualLeave = calcEmployeeAnnualLeave;
window.initAlPage = initAlPage;
window.renderAlCompanyChips = renderAlCompanyChips;
window.selectAlCompany = selectAlCompany;
window.alBackToCompanyList = alBackToCompanyList;
window.renderAlTable = renderAlTable;
window._fmtDate = _fmtDate;
window._fmtLeaveDay = _fmtLeaveDay;
window._buildDaysOptions = _buildDaysOptions;
window.openLeaveLedger = openLeaveLedger;
window._ledgerNavYear = _ledgerNavYear;
window._loadAndRenderLedger = _loadAndRenderLedger;
window._renderLedgerMonthTable = _renderLedgerMonthTable;
window._onLedgerDaysChange = _onLedgerDaysChange;
window._recalcLedgerSum = _recalcLedgerSum;
window.saveLeaveLedger = saveLeaveLedger;
window._syncLedgerToPayrolls = _syncLedgerToPayrolls;
window.openAlPromoModal = openAlPromoModal;
window._buildLeavePromoWorkerBody = _buildLeavePromoWorkerBody;
window._buildLeavePromoCompanyBody = _buildLeavePromoCompanyBody;
window._calcLeaveEndDate = _calcLeaveEndDate;
window.confirmSendLeavePromotion = confirmSendLeavePromotion;
window.loadLeavePromotionHistory = loadLeavePromotionHistory;
window.initLpPage = initLpPage;
window._lpFillCompanyFilter = _lpFillCompanyFilter;
window.renderLpTable = renderLpTable;
window.renderCenHistory = renderCenHistory;
window.setCenHistoryPage = setCenHistoryPage;
window._cenRenderPagination = _cenRenderPagination;
