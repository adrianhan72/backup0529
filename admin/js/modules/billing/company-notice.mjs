/**
 * billing/company-notice.mjs — Phase 6: 완전 변환
...
/** 탭 전환 */
export function cenSwitchTab(tab){
  _cenTab = tab;
  ...
  if(tab==='history'){
    if(!_cenHistoryLoaded){
      (async()=>{ await _w('cenLoadHistory')(true); renderCenHistory(); })();
    } else {
      renderCenHistory();
    }
  }
 ...
 */
export function _cenFillTemplateSampleSel(){
 ...
 */
export function renderCenTemplate(){
  const sel    = document.getElementById('cen-tmpl-sample-sel');
  ...
  if(selVal !== '__demo__'){
    ...
    if(item){
      ...
      coRep       = _w('getCompanyRepName')(item.co) || coRep;
...
export function _cenFillCompanyFilter(selId){
...
/** 통지 이력 DB에서 로드 */
export async function cenLoadHistory(force=false){
 ...
 */
export function _cenGetTargetContracts(){
  ...
  return getContracts().filter(c => {
    ...
    const normalizedCat = _w('normalizeContractType')(cat);
...
/** 요약 통계 업데이트 */
export function renderCenStats(){
...
/** 통지 대상 목록 테이블 렌더링 */
export function renderCenTargetList(){
  ...
  tbody.innerHTML = pageData.map((c, idx) => {
    ...
    return `<tr>
      ...
      <td><span class="badge ${CAT_BADGE_CLS[_w('normalizeContractType')(c._cat)]||'badge-gray'}" style="font-size:11px;">${_w('contractTypeLabel')(c._cat)}</span></td>
...
export function setCenTargetPage(p){ _cenTargetPage = p; renderCenTargetList(); }

/** 선택 건수 업데이트 및 일괄 버튼 상태 */
export function cenUpdateSelectedCount(){
...
export function _cenUpdateBulkBtns(){
  const checked = document.querySelectorAll('.cen-row-chk:checked');
...
/** 전체 선택/해제 */
export function cenToggleAll(chkEl){
...
/** 개별 통지 발송 */
export async function cenSendOne(contractId, method){
  const c   = getContracts().find(x=>x.id===contractId);
  const emp = c ? getEmployees().find(e=>e.id===c.employee_id) : null;
  const co  = c ? getCompanies().find(x=>x.id===c.company_id)  : null;
  if(!c||!emp||!co){ _w('toast')('계약 정보를 찾을 수 없습니다.','error'); return; }

  if(method===DISPATCH_METHOD.KAKAO && !emp.phone){ _w('toast')(`${emp.name} — 전화번호가 등록되지 않았습니다.`,'error'); return; }
  if(method===DISPATCH_METHOD.EMAIL && !emp.email){ _w('toast')(`${emp.name} — 이메일이 등록되지 않았습니다.`,'error'); return; }
  ...
  try{
    ...
    _w('toast')(`✅ ${emp.name} 계약만료 통지(${method}) 완료 + 고객사 알림 발송`, 'success');
    await cenRefresh();
  } catch(e){ console.error(e); _w('toast')('발송 중 오류가 발생했습니다.','error'); }
}

/** 일괄 발송 */
export async function cenBulkSend(method){
  const checked = [...document.querySelectorAll('.cen-row-chk:checked')];
  if(!checked.length){ _w('toast')('발송할 항목을 선택하세요.','warning'); return; }
  ...
  if(!targets.length){ _w('toast')(`${method} 발송 가능한 대상이 없습니다. (연락처 미등록)`, 'warning'); return; }
  ...
  _w('toast')(`일괄 ${method} 완료 — 성공 ${ok}건 + 고객사 알림 발송${fail?` / 실패 ${fail}건`:''}`, ok>0?'success':'error');
  await cenRefresh();
}
 ...
 */
export async function _sendCompanyNotice({
 ...
 */
export async function _cenSendCompanyNotice({ c, emp, co, daysLeft }){
  if(!c || !co) return;
  const adminName = _w('_getAdminUsername')();
  const cat       = emp?.employment_category || c.contract_type || '';
...
/** 통지 이력 DB 저장 */
export async function _cenSaveNotice({ contractId, method, status, recipient, note, daysLeft }){
  const c   = getContracts().find(x=>x.id===contractId);
  const emp = c ? getEmployees().find(e=>e.id===c.employee_id) : null;
  const co  = c ? getCompanies().find(x=>x.id===c.company_id)  : null;
  const adminName = _w('_getAdminUsername')();
...
/** 새로고침 */
export async function cenRefresh(){
  _cenHistoryLoaded = false;
  await _w('cenLoadHistory')(true);
  renderCenTargetList();
  renderCenStats();
  render2YrStats();
...
// ══ window 등록 (레거시 호환) ══
window.cenSwitchTab = cenSwitchTab;
window._cenFillTemplateSampleSel = _cenFillTemplateSampleSel;
window.renderCenTemplate = renderCenTemplate;
window._setText = _setText;
window.initCenPage = initCenPage;
window._cenFillCompanyFilter = _cenFillCompanyFilter;
window.cenLoadHistory = cenLoadHistory;
window._cenGetTargetContracts = _cenGetTargetContracts;
window.renderCenStats = renderCenStats;
window.renderCenTargetList = renderCenTargetList;
window.setCenTargetPage = setCenTargetPage;
window.cenUpdateSelectedCount = cenUpdateSelectedCount;
window._cenUpdateBulkBtns = _cenUpdateBulkBtns;
window.cenToggleAll = cenToggleAll;
window.cenSendOne = cenSendOne;
window.cenBulkSend = cenBulkSend;
window._sendCompanyNotice = _sendCompanyNotice;
window._cenSendCompanyNotice = _cenSendCompanyNotice;
window._cenSaveNotice = _cenSaveNotice;
window.cenRefresh = cenRefresh;
