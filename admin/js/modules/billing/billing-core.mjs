/**
 * billing/billing-core.mjs — Phase 6: 청구 관리 코어
...
export async function generateMonthlyBilling(){
  ...
  try {
    ...
    // 이용중인 고객사만 대상
    const activeCompanies=getCompanies().filter(c=>c.status===COMPANY_STATUS.ACTIVE);
    
    for(const co of activeCompanies){
      // 이미 해당 연월에 청구 데이터가 있는지 확인
      const exists=getPayrolls().find(b=>b.company_id===co.id&&b.billing_year==yr&&b.billing_month==mo);
      if(exists){
      ...
      const activeEmps=getEmployees().filter(e=>
        e.company_id===co.id && 
        (e.status===EMP_STATUS.ACTIVE||e.status===EMP_STATUS.ACTIVE)
      );
      
      // 해당 월 급여 데이터가 있는 직원만 카운트
      const payrollEmps=getPayrolls().filter(p=>
        p.company_id===co.id && 
        p.pay_year==yr && 
        p.pay_month==mo
      );
    ...
    if(newBillings.length===0){
      _w('toast')('생성할 청구 데이터가 없습니다 (이미 생성되었거나 재직 직원 없음)','error');
      return;
    }
    
    // API 호출하여 생성
    for(const bill of newBillings){
      await _w('api')('../tables/billing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(bill)});
    }
    
    await _w('loadBillings')();
    renderBillings();
    _w('renderDashboard')();
    _w('toast')(`✅ ${yr}년 ${mo}월 청구 ${newBillings.length}건 생성 완료`,'success');
    
  } catch(err){
    console.error('[청구 생성 오류]',err);
    _w('toast')('청구 생성 실패: '+err.message,'error');
  }
}

// 고객사별 청구 생성
export async function generateBillingForCompany(companyId, companyName){
  const now=new Date();
  const yr=now.getFullYear();
  const mo=now.getMonth()+1;
  ...
  if(!co){
    _w('toast')('고객사 정보를 찾을 수 없습니다','error');
    return;
  }
  
  if(co.status!==COMPANY_STATUS.ACTIVE){
    _w('toast')('이용중인 고객사만 청구를 생성할 수 있습니다','error');
    return;
  }
  ...
  try {
    // 이미 해당 연월에 청구 데이터가 있는지 확인
    const exists=getPayrolls().find(b=>b.company_id===companyId&&b.billing_year==yr&&b.billing_month==mo);
    if(exists){
      _w('toast')('이미 해당 월에 청구가 생성되어 있습니다','error');
      return;
    }
    ...
    if(empCount===0){
      _w('toast')('재직 직원이 없어 청구를 생성할 수 없습니다','error');
      return;
    }
    ...
    // API 호출하여 생성
    await _w('api')('../tables/billing',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(billing)});
    
    await _w('loadBillings')();
    renderBillings();
    _w('renderDashboard')();
    _w('toast')(`✅ ${coName} ${yr}년 ${mo}월 청구 생성 완료 (${empCount}명 × ${Math.round(amountPerEmp).toLocaleString('ko-KR')}원 = ${Math.round(totalAmount).toLocaleString('ko-KR')}원)`,'success');
...
// 고객사별 누적 미납금 계산 함수
export function calculateTotalUnpaid(companyId, currentBillingYear, currentBillingMonth){
  // 해당 청구보다 이전 월 중 마감일이 지났는데도 미납된 금액만 합산
  const todayStr = new Date().toISOString().slice(0, 10);
  const previousBillings = getPayrolls().filter(b => {
...
// 사용료 관리 상단 통계 업데이트
export function updateBillingStats(){
  const today = new Date();
  const dateStr = `${today.getFullYear()}년 ${today.getMonth() + 1}월 ${today.getDate()}일`;
  
  // 이용 중 고객사 수
  const activeCompanies = getCompanies().filter(c => c.status===COMPANY_STATUS.ACTIVE).length;
  
  // 등록 직원 수 (재직 중인 직원만)
  const activeEmployees = getEmployees().filter(e => e.status===EMP_STATUS.ACTIVE || e.status===EMP_STATUS.ACTIVE).length;
...
export function renderBillings(){
  ...
  if(filterNow === 'virtual'){
    ...
    const hasPartial = getPayrolls().some(b => {
  ...
  // 1. 기존 청구 데이터 (필터 제거)
  let filtered = getPayrolls().slice();
  ...
  if(showUnbilledCompanies){
    ...
    activeCompanies.forEach(co => {
      // 이번 달 청구가 있는지 확인
      const hasBilling = getPayrolls().some(b => 
        b.company_id === co.id && 
  ...
  if(searchQuery){
    filtered = filtered.filter(b => {
      const co = getCompanies().find(c => c.id === b.company_id);
...
// 완납 확인
export async function confirmFullPayment(billingId){
  const bill=getPayrolls().find(b=>b.id===billingId);
  if(!bill) return;
  
  const co=getCompanies().find(c=>c.id===bill.company_id);
  ...
  try {
    ...
    await _w('api')(`../tables/billing/${billingId}`,{
      method:'PUT',
    ...
    renderBillings();
    _w('renderDashboard')();
    _w('toast')('✅ 완납 확인 완료','success');
    
  } catch(err){
    console.error('[완납 확인 오류]',err);
    _w('toast')('완납 확인 실패: '+err.message,'error');
  }
}

// 전체 체크박스 토글
export function toggleAllBillingChecks(){
...
export async function bulkPayment(){
  ...
  if(checkboxes.length === 0){
    _w('toast')('완납 처리할 항목을 선택해주세요','warning');
    return;
  }
  ...
  if(billingIds.length === 0){
    _w('toast')('완납 처리 가능한 청구 데이터가 없습니다','warning');
    return;
  }
  ...
  try {
    ...
    _w('toast')(`✅ ${successCount}건 완납 처리 완료`, 'success');
    
  } catch(err){
    console.error('[일괄 완납 오류]', err);
    _w('toast')('일괄 완납 처리 실패: ' + err.message, 'error');
  }
}

// 선택 일괄 청구 생성
export async function bulkCreateBilling(){
  const checkboxes = document.querySelectorAll('.bill-check:checked');
  
  if(checkboxes.length === 0){
    _w('toast')('청구 생성할 항목을 선택해주세요','warning');
    return;
  }
  ...
  if(virtualItems.length === 0){
    _w('toast')('청구 생성 가능한 청구대상이 없습니다','warning');
    return;
  }
  ...
  try {
    ...
    for(const item of virtualItems){
      const co = getCompanies().find(c => c.id === item.companyId);
      if(!co || co.status!==COMPANY_STATUS.ACTIVE) continue;
      
      // 이미 청구가 있는지 확인
      const exists = getPayrolls().find(b => 
    ...
    _w('toast')(`✅ ${successCount}건 청구 생성 완료`, 'success');
    
  } catch(err){
    console.error('[일괄 청구 생성 오류]', err);
    _w('toast')('일괄 청구 생성 실패: ' + err.message, 'error');
  }
}
...
// 납부이력 모달 열기
export function openPaymentHistoryModal(companyId, companyName){
  // 고객사명 표시
  document.getElementById('ph-company-name').textContent = companyName;
  
  // 해당 고객사의 모든 청구 데이터 가져오기
  const companyBillings = getPayrolls()
...
export function openPartialPaymentModal(billingId, totalAmountOverride){
  const bill=getPayrolls().find(b=>b.id===billingId);
  if(!bill) return;
...
export function openPartialPaymentModalVirtual(companyId, coName, billingYear, billingMonth, totalAmount){
...
export function calculateRemaining(){
  ...
  const bill = currentBillingId ? getPayrolls().find(b=>b.id===currentBillingId) : null;
...
// 일부납 확인
export async function confirmPartialPayment(){
  const bill=getPayrolls().find(b=>b.id===currentBillingId);
  if(!bill) return;
  
  const paymentAmount = parseFloat((document.getElementById('pp-payment-amount').value||'').replace(/,/g,'')) || 0;
  
  if(paymentAmount <= 0){
    _w('toast')('납부 금액을 입력해주세요','error');
    return;
  }
  ...
  try {
    ...
    await _w('api')(`../tables/billing/${currentBillingId}`,{
      method:'PUT',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify(updated)
    });
    ...
    _w('closeModal')('partial-payment-modal');
    
    if(isFullyPaid){
      _w('toast')('✅ 완납 처리 완료','success');
    }else{
      _w('toast')('✅ 일부납 처리 완료','success');
    }
    
  } catch(err){
    console.error('[일부납 처리 오류]',err);
    _w('toast')('일부납 처리 실패: '+err.message,'error');
  }
}
...
// ══ window 등록 (레거시 호환) ══
window.generateMonthlyBilling = generateMonthlyBilling;
window.generateBillingForCompany = generateBillingForCompany;
window.calculateTotalUnpaid = calculateTotalUnpaid;
window.updateBillingStats = updateBillingStats;
window.renderBillings = renderBillings;
window.confirmFullPayment = confirmFullPayment;
window.toggleAllBillingChecks = toggleAllBillingChecks;
window.bulkPayment = bulkPayment;
window.bulkCreateBilling = bulkCreateBilling;
window.openPaymentHistoryModal = openPaymentHistoryModal;
window.openPartialPaymentModal = openPartialPaymentModal;
window.openPartialPaymentModalVirtual = openPartialPaymentModalVirtual;
