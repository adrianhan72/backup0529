/**
 * billing/payslip-send.mjs — Phase 6: 급여명세서 발송
...
// ─── 고객사 선택 칩 렌더 ───
export function renderPssCompanyList(){
...
// ─── 고객사 선택 ───
export async function selectPssCompany(id, name){
  _pssCompanyId   = id;
...
// ─── 고객사 선택 해제 ───
export function clearPssCompanySelect(){
  _pssCompanyId = null;
...
// ─── 발송 이력 로드 (전체, 이 고객사) ───
export async function _pssLoadLogs(){
  const data = await _w('api')(`../tables/payroll_send_logs?limit=500`);
  _pssSendLogs = (data.data || []).filter(l => l.company_id === _pssCompanyId);
}

// ─── 년월 탭 렌더 ───
export function renderPssMonthTabs(){
...
// ─── 년월 선택 ───
export function selectPssYM(year, month){
  _pssYM = { year, month };
  _pssLogPage = 1;
  renderPssMonthTabs();
  renderPssUnsentList();
...
export function _pssGetMonthPayrolls(year, month){
  ...
  return getPayrolls().filter(p => {
    ...
    // 계약이 임시저장 상태인 직원 제외
    const ct = getContracts().find(c => c.employee_id === p.employee_id &&
      (c.status===EMP_STATUS.ACTIVE || c.status===CONTRACT_STATUS.ACTIVE))
      || getContracts().find(c => c.employee_id === p.employee_id);
...
export function _pssGetSentPayrollIds(year, month){
  const yr = year  ?? _pssYM.year;
  const mo = month ?? _pssYM.month;
  return new Set(
    _pssSendLogs
...
// ─── 해당 년월 발송 완료 레코드 목록 ───
export function _pssGetSentPayrolls(year, month){
  const sentIds = _pssGetSentPayrollIds(year, month);
  return _pssGetMonthPayrolls(year, month).filter(p => sentIds.has(p.id));
}

// ─── 해당 년월 미발송 목록 ───
export function _pssGetUnsentList(year, month){
  const sentIds = _pssGetSentPayrollIds(year, month);
  return _pssGetMonthPayrolls(year, month).filter(p => !sentIds.has(p.id));
}

// ─── 미발송 목록 렌더 ───
export function renderPssUnsentList(){
  ...
  tbody.innerHTML = unsentList.map((p, idx) => {
    const emp      = getEmployees().find(e => e.id === p.employee_id) || {};
...
// ─── 미발송 목록 개별 관리 — 알림톡 발송 ───
export async function _pssKakaoSendRow(payrollId, empId){
  const p   = getPayrolls().find(x => x.id === payrollId);
  ...
  if(!phone){
    _w('toast')(`${empName} — 휴대전화번호가 등록되어 있지 않습니다.`, 'error');
    return;
  }
  ...
  try{
    ...
    // ② 카카오 알림톡 발송
    _w('toast')(`${empName} — 알림톡 발송 중...`, 'info');
    ...
    _w('toast')(`✅ ${empName} — ${yr}년 ${mo}월 알림톡 발송 완료`, 'success');
    renderPssUnsentList(); renderPssLogs(); renderPssMonthTabs(); _pssUpdateStats();
  } catch(err){
    console.error('[알림톡 발송 실패]', empName, err);
    _w('toast')(`✖ ${empName} — 알림톡 발송 실패`, 'error');
  }
}

// ─── 미발송 목록 개별 관리 — 이메일 발송 ───
export async function _pssEmailSendRow(payrollId, empId){
  const p   = getPayrolls().find(x => x.id === payrollId);
  const emp = getEmployees().find(e => e.id === empId) || {};
  if(!p) return;
  ...
  if(!email.trim()){
    _w('toast')(`${empName} — 이메일 주소가 등록되어 있지 않습니다.`, 'error');
    return;
  }
  ...
  try{
    ...
    _w('toast')(`✖ ${empName} — 이메일 발송 실패`, 'error');
  }
}

// ─── 미발송 목록 개별 관리 — 수동 교부 완료 ───
export async function _pssManualDoneRow(payrollId, empId){
  const p   = getPayrolls().find(x => x.id === payrollId);
  const emp = getEmployees().find(e => e.id === empId) || {};
  if(!p) return;
  ...
  try{
    ...
    _w('toast')(`✔ ${empName} — 수동 교부 완료 처리됐습니다.`, 'success');
    renderPssUnsentList(); renderPssLogs(); renderPssMonthTabs(); _pssUpdateStats();
  } catch(err){
    console.error('[수동 교부 저장 실패]', empName, err);
    _w('toast')('저장 중 오류가 발생했습니다.', 'error');
  }
}

// ─── 요약 통계 업데이트 ───
export function _pssUpdateStats(){
  // ── 대시보드 미발송 알림 배너 업데이트 ──
  _updateDashUnsentBanner();
}

export function _updateDashUnsentContractBanner(){
...
export function _updateDashUnsentBanner(){
  const section = document.getElementById('dash-unsent-section');
  if(!section) return;

  // 전체 고객사 × 전체 월 미발송 합산 (_allSendLogs 사용)
  const sentPayrollIds = new Set(_allSendLogs.map(l => l.payroll_id));
  const totalUnsent = getPayrolls().filter(p => !sentPayrollIds.has(p.id)).length;
...
export function renderPssLogs(){
  ...
  if(q){
    logs = logs.filter(l => {
      const emp = getEmployees().find(e => e.id === l.employee_id) || {};
  ...
  tbody.innerHTML = paged.map(l => {
    ...
    return `<tr>
      ...
      <td style="font-size:12px;color:#6b7280;">${_w('_resolveAdminName')(l.sent_by)||'-'}</td>
...
export function setPssLogPage(p){ _pssLogPage = p; renderPssLogs(); }
export function clearPssLogSearch(){ const el = document.getElementById('pss-log-search'); if(el) el.value=''; _pssLogPage=1; renderPssLogs(); }

// ─── 단건 발송 모달 열기 ───
export function openPssConfirmModal(payrollId, empId, year, month){
  const p   = getPayrolls().find(x => x.id === payrollId);
  const emp = getEmployees().find(e => e.id === empId) || {};
  if(!p) return;
  ...
  document.getElementById('pss-confirm-info').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:6px;">
      <div><span style="color:#7c3aed;font-weight:700;font-size:13.5px;">${emp.name||'-'}</span>
        <span class="badge ${_w('empCatBadge')(emp.employment_category||'-')}" style="font-size:10.5px;padding:2px 7px;margin-left:6px;">${_w('contractTypeLabel')(emp.employment_category) || '-'}</span>
      </div>
      <div style="color:#374151;">📅 <b>${yr}년 ${moStr}월</b> 급여명세서</div>
      <div style="color:#374151;">💰 실수령액 <b>${_w('won2')(p.net_pay)}원</b></div>
...
export function closePssConfirmModal(){
...
// ─── 발송 방법 선택 버튼 토글 ───
export function selectPssMethod(method){
  _pssSelectedMethod = method;
  _pssRefreshMethodBtns('pss-method');
}
export function selectPssBulkMethod(method){
  _pssBulkMethod = method;
  _pssRefreshMethodBtns('pss-bulk-method');
}
export function _pssRefreshMethodBtns(prefix){
...
// ─── 단건 발송 처리 확정 ───
export async function confirmPssSend(){
  ...
  if(method !== 'kakao'){
    ...
    try{
      ...
      _w('toast')(`✅ ${empName} — ${year}년 ${month}월 발송 처리 완료`, 'success');
      renderPssUnsentList(); renderPssLogs(); renderPssMonthTabs(); _pssUpdateStats();
    } catch(e){
      _w('toast')('발송 처리 중 오류가 발생했습니다.', 'error');
      console.error(e);
    }
    return;
  }
  ...
  try{
    ...
    _pssSingleSendRunning = false;
    closePssConfirmModal();
    _w('toast')(`✅ ${empName} — ${year}년 ${month}월 카카오 발송 완료`, 'success');
    renderPssUnsentList(); renderPssLogs(); renderPssMonthTabs(); _pssUpdateStats();
...
/* 직원별 상태 테이블 초기 렌더 */
export function _pssBulkRenderTable(){
...
/* 상태 배지 HTML */
export function _pssBulkStatusHtml(status){
  const map = {
    idle:       ['idle',       '⏳ 대기'],
...
/* 행 상태 업데이트 */
export function _pssBulkSetStatus(i, status){
...
/* 진행바 업데이트 */
export function _pssBulkUpdateProgress(){
...
// ─── 일괄 발송 모달 열기 ───
export async function openPssSendAllModal(){
  const unsentList = _pssGetUnsentList(_pssYM.year, _pssYM.month);
  const targets    = unsentList;

  if(!targets.length){ _w('toast')('발송 대상이 없습니다.', 'error'); return; }
...
export function closePssBulkModal(){
...
/* 개별 이메일 발송 */
export async function _pssEmailSend(i){
  ...
  try{
    ...
    _w('toast')(`✖ ${item.empName} 이메일 발송 실패`, 'error');
    _pssBulkRenderTable(); // 버튼 복원
  }
}

/* 개별 수동 교부 완료 처리 */
export async function _pssManualDone(i){
  const item = _pssBulkSendItems[i];
  if(!item) return;
  ...
  try{
    ...
    _pssBulkRenderTable();
    _pssCheckAllDone();
    _w('toast')(`✔ ${item.empName} 수동 교부 완료 처리됐습니다.`, 'success');
    renderPssUnsentList(); renderPssLogs(); renderPssMonthTabs(); _pssUpdateStats();
  } catch(err){
...
export function _pssCheckAllDone(){
...
export async function confirmPssBulkSend(){
  if(_pssBulkSendRunning) return;
  if(!_pssBulkSendItems.length){ _w('toast')('발송 대상이 없습니다.', 'error'); return; }
  ...
  for(let i = 0; i < _pssBulkSendItems.length; i++){
    ...
    try{
      await _w('api')('../tables/payroll_send_logs', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(logBody) });
      _pssSendLogs.push(logBody);
      _allSendLogs.push(logBody);
  ...
  if(failCnt === 0){
    ...
    _w('toast')(`✅ 전체 ${successCnt}명 알림톡 발송 완료!`, 'success');
  } else {
    sendBtn.disabled  = false;
    sendBtn.style.background = '';
    sendBtn.style.color = '';
    ...
    _w('toast')(`⚠ 발송 완료: ${successCnt}명 성공, ${failCnt}명 실패`, 'error');
  }
...
// ══ window 등록 (레거시 호환) ══
window.renderPssCompanyList = renderPssCompanyList;
window.selectPssCompany = selectPssCompany;
window.clearPssCompanySelect = clearPssCompanySelect;
window._pssLoadLogs = _pssLoadLogs;
window.renderPssMonthTabs = renderPssMonthTabs;
window.selectPssYM = selectPssYM;
window._pssGetMonthPayrolls = _pssGetMonthPayrolls;
window._pssGetSentPayrollIds = _pssGetSentPayrollIds;
window._pssGetSentPayrolls = _pssGetSentPayrolls;
window._pssGetUnsentList = _pssGetUnsentList;
window.renderPssUnsentList = renderPssUnsentList;
window._pssKakaoSendRow = _pssKakaoSendRow;
window._pssEmailSendRow = _pssEmailSendRow;
window._pssManualDoneRow = _pssManualDoneRow;
window._pssUpdateStats = _pssUpdateStats;
window._updateDashUnsentContractBanner = _updateDashUnsentContractBanner;
window._updateDashUnsentBanner = _updateDashUnsentBanner;
window.renderPssLogs = renderPssLogs;
window.setPssLogPage = setPssLogPage;
window.clearPssLogSearch = clearPssLogSearch;
window.openPssConfirmModal = openPssConfirmModal;
window.closePssConfirmModal = closePssConfirmModal;
window.selectPssMethod = selectPssMethod;
window.selectPssBulkMethod = selectPssBulkMethod;
window._pssRefreshMethodBtns = _pssRefreshMethodBtns;
window.confirmPssSend = confirmPssSend;
window._pssBulkRenderTable = _pssBulkRenderTable;
window._pssBulkStatusHtml = _pssBulkStatusHtml;
window._pssBulkSetStatus = _pssBulkSetStatus;
window._pssBulkUpdateProgress = _pssBulkUpdateProgress;
window.openPssSendAllModal = openPssSendAllModal;
window.closePssBulkModal = closePssBulkModal;
window._pssEmailSend = _pssEmailSend;
window._pssManualDone = _pssManualDone;
window._pssCheckAllDone = _pssCheckAllDone;
window.confirmPssBulkSend = confirmPssBulkSend;
