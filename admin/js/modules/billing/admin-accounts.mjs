/**
 * billing/admin-accounts.mjs — Phase 6: 관리자계정
...
export function _aaRenderTable(){
  const tbody           = document.getElementById('aa-tbody');
...
export function openAdminAccountModal(){
  if(sessionStorage.getItem('admin_username') !== 'admin'){
    _w('toast')('마스터 관리자만 신규 계정을 등록할 수 있습니다.', 'error');
    return;
  }
  ...
  _w('openModal')('admin-account-modal');
  setTimeout(()=>document.getElementById('aa-input-id').focus(), 150);
}

/* ── 모달 내 눈 토글 ── */
export function aaTogglePw(inputId, iconId){
...
/* ── 인라인 에러 표시 헬퍼 ── */
export function _aaFieldErr(inputId, errId, msg){
...
export function _aaFieldOk(inputId, errId){
  const inp = document.getElementById(inputId);
  const err = document.getElementById(errId);
...
export async function saveAdminAccount(){
  ...
  try{
    ...
    _w('closeModal')('admin-account-modal');
    _w('toast')('계정이 등록되었습니다.', 'success');
    await renderAdminAccounts();
  } catch(e){
    _w('toast')('저장에 실패했습니다.', 'error');
  } finally{
...
/* ── 계정 삭제 ── */
export async function deleteAdminAccount(id, username){
  if(username === 'admin'){
    _w('toast')('마스터 관리자 계정은 삭제할 수 없습니다.', 'error'); return;
  }
  if(_aaAccounts.length <= 1){
    _w('toast')('마지막 계정은 삭제할 수 없습니다.', 'error'); return;
  }
  if(!confirm(`'${username}' 계정을 삭제하시겠습니까?\n삭제 후 해당 계정으로 로그인할 수 없습니다.`)) return;
  try{
    await fetch('../tables/admin_accounts/' + id, { method: 'DELETE' });
    _w('toast')('계정이 삭제되었습니다.', 'success');
    await renderAdminAccounts();
  } catch(e){
...
export function openChangeNameModal(id, currentName){
  if(sessionStorage.getItem('admin_username') !== 'admin'){
    _w('toast')('마스터 관리자만 표시 이름을 변경할 수 있습니다.', 'error'); return;
  }
  _cnmTargetId = id;
  ...
  _w('openModal')('aa-name-change-modal');
  setTimeout(()=>{ const el = document.getElementById('aa-name-input'); if(el){ el.focus(); el.select(); } }, 150);
}

/* ── 표시 이름 변경 저장 ── */
export async function submitChangeName(){
  if(sessionStorage.getItem('admin_username') !== 'admin'){
    _w('toast')('마스터 관리자만 표시 이름을 변경할 수 있습니다.', 'error'); return;
  }
  if(!_cnmTargetId){ _w('toast')('대상 계정 정보가 없습니다.', 'error'); return; }
  ...
  try{
    ...
    // ① 로컬 캐시 동기화 (window.allAdminAccounts)
    const cached = window.allAdminAccounts.find(a => a.id === _cnmTargetId);
    ...
    _w('closeModal')('aa-name-change-modal');
    _w('toast')('표시 이름이 변경되었습니다.', 'success');
    _cnmTargetId = null;
...
export function openChangePwModal(id, username, displayName){
  // 마스터 세션 이중 검사
  if(sessionStorage.getItem('admin_username') !== 'admin'){
    _w('toast')('마스터 관리자만 비밀번호를 변경할 수 있습니다.', 'error'); return;
  }
  _cpwTargetId = id;
  ...
  _w('openModal')('aa-pw-change-modal');
  setTimeout(()=> document.getElementById('aa-cpw-new').focus(), 150);
}

/* ── 비밀번호 변경 인라인 에러 헬퍼 ── */
export function _cpwFieldErr(inputId, errId, msg){
  const inp = document.getElementById(inputId);
  const err = document.getElementById(errId);
...
export function _cpwFieldOk(inputId, errId){
...
export async function submitChangePw(){
  ...
  if(!_cpwTargetId){ _w('toast')('대상 계정 정보가 없습니다.', 'error'); return; }
  ...
  try{
    ...
    _w('closeModal')('aa-pw-change-modal');
    _w('toast')('비밀번호가 변경되었습니다.', 'success');
    _cpwTargetId = null;
  } catch(e){
    _w('toast')('저장에 실패했습니다.', 'error');
    btn.disabled = false;
...
window._aaRenderTable = _aaRenderTable;
window._esc = _esc;
window.openAdminAccountModal = openAdminAccountModal;
window.aaTogglePw = aaTogglePw;
window._aaFieldErr = _aaFieldErr;
window._aaFieldOk = _aaFieldOk;
...
window._cpwFieldErr = _cpwFieldErr;
window._cpwFieldOk = _cpwFieldOk;
