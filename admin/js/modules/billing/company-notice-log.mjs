/**
 * billing/company-notice-log.mjs — Phase 6: 알림로그
...
/** 고객사 드롭다운 옵션 채우기 */
export function _cnlPopulateCompanySelect(){
...
/** 페이지 진입 초기화 */
export async function initCnlPage(){
  _cnlPopulateCompanySelect();
  // 전역 고객사 선택 공유: 다른 페이지에서 선택된 고객사가 있으면 드롭다운도 맞춤
  if(window.currentGlobalCompanyId && !_cnlCompanyId){
    _cnlCompanyId = window.currentGlobalCompanyId;
    const sel = document.getElementById('cnl-filter-company');
...
/** 고객사 드롭다운 변경 핸들러 */
export async function cnlOnCompanyChange(){
  const sel = document.getElementById('cnl-filter-company');
...
export function _cnlShowLoading(show){
...
export async function cnlLoadData(){
  _cnlShowLoading(true);
  try {
    const res = await _w('api')(`../tables/company_notices?limit=1000&sort=sent_at`);
...
/** 새로고침 */
export async function cnlReload(){
  _cnlLoaded = false;
...
export function renderCnlReserveCard(){
  ...
  tbody.innerHTML = list.map((n, idx) => {
    ...
    return `<tr style="background:${rowBg};border-bottom:1px solid #fef3c7;"
      ...
      <td style="padding:10px 14px;font-size:12px;color:#64748b;white-space:nowrap;">${_w('_resolveAdminName')(n.sent_by)||'-'}</td>
...
export function openCnlDetailById(recordId){
...
/** 테이블 렌더 */
export function renderCnlTable(){
...
/** 페이지네이션 렌더 */
export function renderCnlPagination(total){
...
/** 상세 모달 열기 */
export function openCnlDetail(listIdx){
  const filterCompany = document.getElementById('cnl-filter-company')?.value || '';
  const filterType    = document.getElementById('cnl-filter-type')?.value || '';
  const searchQ       = (document.getElementById('cnl-search')?.value || '').trim().toLowerCase();
  const list = _cnlList.filter(n => {
...
/** 상세 모달 닫기 */
export function closeCnlDetailModal(){
  const modal = document.getElementById('cnl-detail-modal');
...
// ══ window 등록 (레거시 호환) ══
window._cnlPopulateCompanySelect = _cnlPopulateCompanySelect;
window.initCnlPage = initCnlPage;
window.cnlOnCompanyChange = cnlOnCompanyChange;
window._cnlShowLoading = _cnlShowLoading;
window.cnlLoadData = cnlLoadData;
window.cnlReload = cnlReload;
window.renderCnlReserveCard = renderCnlReserveCard;
window.openCnlDetailById = openCnlDetailById;
window.renderCnlTable = renderCnlTable;
window.renderCnlPagination = renderCnlPagination;
window.openCnlDetail = openCnlDetail;
window.closeCnlDetailModal = closeCnlDetailModal;
