/**
 * billing/general-notice.mjs — Phase 6: 일반알림
...
export function _toDatetimeLocalValue(d){
...
// 페이지 초기화
// ─────────────────────────────────────────────
export async function initGnPage(){
...
// 고객사 칩 렌더
// ─────────────────────────────────────────────
export function renderGnCompanyChips(){
...
export function gnDeselectAll(){
  _gnSelectedIds.clear();
  renderGnCompanyChips();
}

export function _updateGnSelectBtn(){
  const q = (document.getElementById('gn-company-search')?.value || '').toLowerCase();
  const total = getCompanies().filter(c => !c.is_draft && c.status===COMPANY_STATUS.ACTIVE && (!q || (c.company_name||'').toLowerCase().includes(q))).length;
...
export function _updateGnSelectedCount(){
...
// 공지 작성 모달
// ─────────────────────────────────────────────
export function openGnComposeModal(){
  if(_gnSelectedIds.size === 0){ _w('toast')('고객사를 먼저 선택하세요.','error'); return; }
  ...
  if(targets){
    targets.innerHTML = [..._gnSelectedIds].map(id => {
      const co = getCompanies().find(x => x.id === id);
...
/** 예약 메시지 수정 모달 오픈 */
export async function openGnEditModal(recordId){
  const n = _cnlList.find(x => x.id === recordId);
  if(!n){ _w('toast')('레코드를 찾을 수 없습니다.', 'error'); return; }
  if(n.gn_status !== 'scheduled'){
    _w('toast')('이미 발송되었거나 취소된 공지는 수정할 수 없습니다.', 'error');
    return;
  }
...
export function closeGnComposeModal(){
  document.getElementById('gn-compose-modal').style.display = 'none';
}

export function toggleGnSchedule(){
...
// 공지 제출 (즉시 / 예약)
// ─────────────────────────────────────────────
export async function submitGnNotice(){
  const modal     = document.getElementById('gn-compose-modal');
  ...
  if(!title) return _w('toast')('제목을 입력하세요.','error');
  if(!body)  return _w('toast')('본문을 입력하세요.','error');
  if(isSchedule){
    if(!scheduledAt) return _w('toast')('예약 발송 일시를 설정하세요.','error');
    const schedDt = new Date(scheduledAt);
    if(schedDt <= new Date()) return _w('toast')('예약 일시는 현재 시각 이후로 설정하세요.','error');
  }

  // ── 수정 모드: 기존 예약 레코드 PATCH ──
  if(window.editId){
    try {
      await fetch(`../tables/company_notices/${window.editId}`, {
        method : 'PATCH',
      ...
      _gnScheduleTimers = [];
      const n = _cnlList.find(x => x.id === window.editId);
      if(n){
        const co = { id: n.company_id, name: n.company_name };
        const adminName = _w('_getAdminUsername')();
        const delay = new Date(scheduledAt).getTime() - Date.now();
        if(delay > 0){
          const timerId = setTimeout(() => _gnFireScheduled(window.editId, co, title, body, adminName), delay);
      ...
      _w('toast')('✅ 예약 공지가 수정되었습니다.', 'success');
    } catch(e){
      console.error('[예약 수정 오류]', e);
      _w('toast')('수정 중 오류가 발생했습니다.', 'error');
      return;
    }
    closeGnComposeModal();
  ...
  if(isSchedule){
    ...
    _w('toast')(`✅ ${targets.length}개 고객사에 예약 발송이 등록되었습니다.`, 'success');
  } else {
    // ── 즉시 발송 ──
    ...
    _w('toast')(`✅ ${targets.length}개 고객사에 공지를 즉시 발송했습니다.`, 'success');
  }
...
// 예약 발송 실행 (타이머 콜백)
// ─────────────────────────────────────────────
export async function _gnFireScheduled(recordId, co, title, body, adminName){
  const nowISO = new Date().toISOString();
  try {
    // 기존 레코드를 sent 상태로 패치
    ...
    _w('toast')(`📣 [${co.name}] 예약 공지가 발송되었습니다.`, 'success');
...
// ─────────────────────────────────────────────
export async function _gnRestoreScheduledTimers(){
  try {
    const res  = await _w('api')('../tables/company_notices?limit=500');
  ...
  } catch(e){ /* DB 테이블 미존재: 예약 없음, 조용히 처리 */ }
}
...
export function _gnStartPolling(){
...
// 예약 취소
// ─────────────────────────────────────────────
export async function cancelGnScheduled(recordId){
  ...
  try {
    ...
    _w('toast')('예약 공지가 취소되었습니다.', 'success');
    // CNL 페이지에서 호출되므로 cnlReload()로 갱신
    await cnlReload();
  } catch(e){ _w('toast')('취소 처리 중 오류가 발생했습니다.','error'); }
}

// ─────────────────────────────────────────────
// 산정기준 업데이트 시 전체 고객사 중요공지 자동 발송
// ─────────────────────────────────────────────
export async function _gnSendStandardsUpdateNotice(updateType, detail){
  const adminName = _w('_getAdminUsername')();
  const targets   = getCompanies().filter(c => !c.is_draft && c.status===COMPANY_STATUS.ACTIVE);
...
${window._BRAND_SIG}`;
  ...
  _w('toast')(`📣 ${cnt}개 고객사에 산정기준 업데이트 공지를 발송했습니다.`, 'success');
}
...
// ══ window 등록 (레거시 호환) ══
window._toDatetimeLocalValue = _toDatetimeLocalValue;
window.initGnPage = initGnPage;
window.renderGnCompanyChips = renderGnCompanyChips;
window.toggleGnCompany = toggleGnCompany;
window.gnSelectAll = gnSelectAll;
window.gnDeselectAll = gnDeselectAll;
window._updateGnSelectBtn = _updateGnSelectBtn;
window._updateGnSelectedCount = _updateGnSelectedCount;
window.openGnComposeModal = openGnComposeModal;
window.openGnEditModal = openGnEditModal;
window.closeGnComposeModal = closeGnComposeModal;
window.toggleGnSchedule = toggleGnSchedule;
window.submitGnNotice = submitGnNotice;
window._gnFireScheduled = _gnFireScheduled;
window._gnRestoreScheduledTimers = _gnRestoreScheduledTimers;
window._gnStartPolling = _gnStartPolling;
window.cancelGnScheduled = cancelGnScheduled;
window._gnSendStandardsUpdateNotice = _gnSendStandardsUpdateNotice;
