/**
 * standards-full.mjs — Phase 6: 기준정보
...
window._allMinimumWages = []; // cross-module bridge

export async function loadStandards(){
  try{
    const [rd, wd] = await Promise.all([
      _w('api')('../tables/insurance_rates?limit=200'),
      _w('api')('../tables/minimum_wages?limit=100')
    ]);
    window._allInsuranceRates = (rd.data||[]).sort((a,b)=> b.year - a.year || (b.period_start||'').localeCompare(a.period_start||''));
    _allMinimumWages   = (wd.data||[]).sort((a,b)=> b.year - a.year);
    window._allMinimumWages = _allMinimumWages; // sync
  }catch(e){ /* DB 테이블 미존재: 빈 데이터로 조용히 처리 */ }
}
...
// ── 4대보험 요율표 렌더링 ──
export function renderInsuranceRates(){
  const now      = new Date();
  ...
  Object.entries(typeMap).forEach(([type, tableId])=>{
    ...
    if(!tbody) return;
    const rows = window._allInsuranceRates.filter(r => r.insurance_type === type);
...
// ── 최저임금표 렌더링 ──
export function renderMinimumWages(){
...
export function _getTaxForDep(row, dep){
  if(row.length <= 4){
...
export function _isNewTaxFormat(year){ return (year >= 2025); }

export function renderTaxBracketPanel(){
  const curYear = new Date().getFullYear();
...
export function selectTaxBracketYear(y){
...
export function selectTaxBracketDep(dep){
...
export function _renderTaxBracketTable(year){
...
export function searchTaxBracket(){
...
// ── 4대보험 요율 저장 ──
export async function saveInsuranceRate(){
  ...
  if(!year||isNaN(year)) return _w('toast')('적용 연도를 입력하세요.','error');
  if(!start||!end)       return _w('toast')('적용 기간을 입력하세요.','error');
  if(!rate||isNaN(rate)) return _w('toast')('요율을 입력하세요.','error');
  ...
  const existing = window._allInsuranceRates.find(r=>r.id===id);
  ...
  if(existing){
    await _w('api')(`../tables/insurance_rates/${existing.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  } else {
    await _w('api')('../tables/insurance_rates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  }
  await loadStandards();
  ...
  _w('toast')('요율이 업데이트 되었습니다. ✔','success');
...
export async function saveMinimumWage(){
  ...
  if(!year||isNaN(year))     return _w('toast')('연도를 입력하세요.','error');
  if(!hourly||isNaN(hourly)) return _w('toast')('시급을 입력하세요.','error');
  if(!monthly||isNaN(monthly)) return _w('toast')('월급여를 입력하세요.','error');
  const id = `mw_${year}`;
  const existing = _allMinimumWages.find(w=>w.id===id);
  const body = { id, year, hourly_wage:hourly, monthly_wage:monthly, note };
  if(existing){
    await _w('api')(`../tables/minimum_wages/${existing.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  } else {
    await _w('api')('../tables/minimum_wages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  }
  await loadStandards();
  renderMinimumWages();
  ['mw-new-year','mw-new-hourly','mw-new-monthly','mw-new-note'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
  _w('toast')('최저임금이 업데이트 되었습니다. ✔','success');
...
// ── 대시보드 산정기준 배너 ──
export function _renderStandardsBanner(){
  ...
  const hasPension = window._allInsuranceRates.some(r=>
  ...
  if(!hasPension){
    ...
    const hasNext = window._allInsuranceRates.some(r=>r.insurance_type==='national_pension' && r.period_start===nextStart);
  ...
  const hasHealth = window._allInsuranceRates.some(r=>
  ...
  if(!hasHealth){
    ...
    const hasNextHealth = window._allInsuranceRates.some(r=>r.insurance_type==='health' && Number(r.year)===(curYear+1));
...
window.switchStdTab = switchStdTab;
window.renderInsuranceRates = renderInsuranceRates;
window.renderMinimumWages = renderMinimumWages;
window._getTaxForDep = _getTaxForDep;
window._isNewTaxFormat = _isNewTaxFormat;
window.renderTaxBracketPanel = renderTaxBracketPanel;
window.selectTaxBracketYear = selectTaxBracketYear;
window.selectTaxBracketDep = selectTaxBracketDep;
window._renderTaxBracketTable = _renderTaxBracketTable;
window.searchTaxBracket = searchTaxBracket;
window.saveInsuranceRate = saveInsuranceRate;
window.saveMinimumWage = saveMinimumWage;
window._renderStandardsBanner = _renderStandardsBanner;
