 ...
 */
export function _isPIMonthFullyPaid(coId, yr, mo){
 ...
 */
export function _syncXlFields(coId, yr, mo){
 ...
 *    4b. 찾지 못하면 안내 토스트
 */
export async function downloadPrevMonthExcel(){
  ...
  if(!selYr || !selMo){ _w('toast')('년도와 월을 선택하세요.','warning'); return; }
  ...
  if(_isPIMonthFullyPaid(coId, prevYr, prevMo)){
    ...
    _w('toast')(`${prevYr}년 ${prevMo}월 임금대장 엑셀 생성 중…`, 'success');
  ...
  if(!foundYr){
    _w('toast')(`전월(${prevYr}년 ${prevMo}월) 임금대장이 존재하지 않으며, 대체할 수 있는 완료된 임금대장도 없습니다.`, 'warning');
    return;
  }
  ...
  _w('toast')(`${foundYr}년 ${foundMo}월 임금대장 엑셀 생성 중…`, 'success');
  await downloadPayrollExcel();
}
 ...
 */
export function _showPIDownloadFallbackConfirm(prevYr, prevMo, foundYr, foundMo, coName){
...
// ===============================================================================
export async function _probAutoCreateAndSave(){
  ...
  try {
    ...
    _w('toast')(`채용확정 계약서 생성 완료 (${confirmedType})\n이제 급여를 저장합니다...`, 'info');

    // ── 4단계: 급여 저장 (savePI 호출) ──
    // 산정기준 체크 먼저
    const stdCheck = _checkPIStandardsReady(yr, mo, coId || window.currentGlobalCompanyId);
    if(!stdCheck.ok){
    ...
    // 중복 체크 (임시저장 레코드 제외)
    const dup = getPayrolls().find(p => !p.is_draft && p.employee_id === empId && p.pay_year === yr && p.pay_month === mo);
    if(dup){
...
export function goPayrollInputNew(companyId, employeeId, year, month){
...
export function editPayroll(payrollId){
  const p=getPayrolls().find(x=>x.id===payrollId);
  if(!p){_w('toast')('급여 데이터를 찾을 수 없습니다.','error');return;}
  const emp=getEmployees().find(x=>x.id===p.employee_id)||{};
  ...
  // 고객사 칩 UI 전환
  const coEdit = getCompanies().find(x=>x.id===p.company_id);
  if(coEdit){
    document.getElementById('pi-company-select-card').style.display='none';
    document.getElementById('pi-input-section').style.display='';
  ...
  const _editEmpContract = (getContracts()||[]).find(c => c.employee_id === p.employee_id && (c.status===CONTRACT_STATUS.ACTIVE||c.status===EMP_STATUS.ACTIVE));
    ...
    if(_ppElEdit && piContract){
      ...
      if(_isPartialTargetEdit && _endRawEdit){
        ...
        if(_endDateEdit < _monthEndEdit){
          const _coIdEdit = window.currentGlobalCompanyId || document.getElementById('pi-company')?.value;
          const _coEdit   = getCompanies().find(c => c.id === _coIdEdit);
  ...
  _w('toast')(`${emp.name||''} ${p.pay_year}년 ${p.pay_month}월 급여 수정 모드로 진입했습니다.`,'info');
  // 페이지 상단으로 스크롤
 ...
 */
export function _fillPayrollFields(p, cfgCo){
  ...
  const _editCo = getCompanies().find(x=>x.id===p.company_id);
...
export function restoreEditPayroll(){
  if(!piEditPayrollId){
    _w('toast')('수정 모드가 아닙니다.', 'error');
    return;
  }
  if(!confirm('현재 입력 내용을 버리고 원본 데이터로 되돌리겠습니까?')) return;

  const p = getPayrolls().find(x => x.id === piEditPayrollId);
  ...
  const _restoreCo = getCompanies().find(x => x.id === p.company_id);
  ...
  _updatePIBottomBtns();
  _hidePrevMemoBanner();
  _w('toast')('원본 데이터로 복원했습니다.', 'info');
  window.scrollTo({top:0, behavior:'smooth'});
}

// ─── 수정 취소 ───
export async function cancelEditPayroll(){
...
export function validateAndParseExcel(wb, fileName){
  ...
  // ── 연도별 요율 조회 헬퍼 ──
  // window._allInsuranceRates: [{insurance_type, year, period_start, period_end, rate, ...}]
  function getRateForYearMonth(type, year, month){
    const dateStr = `${year}-${String(month).padStart(2,'0')}-01`;
    // 기간이 있는 경우 기간 내 매칭
    const byPeriod = window._allInsuranceRates.find(r =>
    ...
    // 기간 없이 연도만 있는 경우
    const byYear = window._allInsuranceRates.find(r =>
  ...
  let co = getCompanies().find(c => c.company_name === targetCoName);
  if(!co && targetCoName){
    co = getCompanies().find(c =>
    ...
    else   errors.push(`❌ 고객사를 찾을 수 없습니다: "${targetCoName}"\n등록된 고객사: ${getCompanies().map(c=>c.company_name).join(', ')}`);
  }
  ...
  if(isCardFormat){
    ...
    // ── 직원 명단 검증 (카드형) ──
    const coEmps     = getEmployees().filter(e => e.company_id===co.id && (e.status===EMP_STATUS.ACTIVE||e.status===EMP_STATUS.ACTIVE));
    ...
    cardDataRows.forEach((row, di) => {
      ...
      const ct = getContracts().find(c => c.employee_id===emp.id &&
...
export function showUploadReport(canSave, errors, warnings, calcErrors, fixedErrors, validRows){
  // 인자 누락 방어
  errors      = errors      || [];
  warnings    = warnings    || [];
  ...
  if(_uploadParsed && _uploadParsed.allRows){
    const coEmps = getEmployees().filter(e =>
  ...
  _w('openModal')('upload-report-modal');
}

export async function confirmBulkUpload(){
  ...
  for(const r of validRows){
    try{
      // 중복 체크 (임시저장 제외)
      const dup=getPayrolls().find(p=>!p.is_draft&&p.employee_id===r.emp.id&&p.pay_year===year&&p.pay_month===month);
      if(dup){
        await _w('api')(`../tables/payrolls/${dup.id}`,{method:'DELETE'});
        overwritten++;
      }
  ...
  _w('renderPayrolls')();
  _w('renderDashboard')();
  _w('closeModal')('upload-report-modal');
  _uploadParsed=null;
...
export function openExcelModal(){
  ...
  sel.innerHTML='<option value="">-- 고객사를 선택하세요 --</option>'+
    getCompanies().map(c=>`<option value="${c.id}">${c.company_name}</option>`).join('');
  ...
  if(window.currentPayCompanyId){
    sel.value = window.currentPayCompanyId;
    // 고객사가 선택된 경우: 드롭다운 숨기고 표시 배너 보이기
    const co = getCompanies().find(c=>c.id===window.currentPayCompanyId);
  ...
  if(infoBox && window.currentPayCompanyId){
    const co2 = getCompanies().find(c=>c.id===window.currentPayCompanyId);
  ...
  _w('openModal')('excel-modal');
}

export function onXlCompanyChange(){
  ...
  const emps=getEmployees().filter(e=>e.company_id===coId&&(e.status===EMP_STATUS.ACTIVE||e.status===EMP_STATUS.ACTIVE));
...
export async function downloadPayrollExcel(){
  ...
  const coId = (typeof window.currentPayCompanyId !== 'undefined' && window.currentPayCompanyId)
    ? window.currentPayCompanyId
  ...
  if(!coId) return _w('toast')('고객사를 선택하세요.','error');
  if(!yr||!mo) return _w('toast')('년도와 월을 선택하세요.','error');

  const co=getCompanies().find(c=>c.id===coId);
  const emps=getEmployees().filter(e=>e.company_id===coId&&(e.status===EMP_STATUS.ACTIVE||e.status===EMP_STATUS.ACTIVE));
  if(!emps.length) return _w('toast')('재직 직원이 없습니다.','error');

  _w('toast')('엑셀 파일 생성 중...','success');
  ...
  data1.push([
    txt(`사업자번호: ${co.business_number||'-'}  /  대표자: ${_w('getCompanyRepName')(co)}  /  급여지급일: ${co.pay_day||'-'}일  /  산정기간: ${co.pay_period_month&&co.pay_period_day?`${co.pay_period_month} ${co.pay_period_day}일부터 1개월간`:(co.pay_period||'-')}`,C.COINFO_BG,C.COINFO_FG,false,9)
  ]);
  data1.push([txt('')]); // 공백
  ...
  /* 데이터 행 */
  emps.forEach((e,idx)=>{
    const ct=getContracts().find(c=>c.employee_id===e.id&&(c.status===CONTRACT_STATUS.ACTIVE||c.status===EMP_STATUS.ACTIVE))||null;
    const py=fillExisting?getPayrolls().find(p=>p.employee_id===e.id&&p.pay_year===yr&&p.pay_month===mo):null;
  ...
  emps.forEach((e,idx)=>{
    ...
    ds.push([
      ...
      sl(_w('getCompanyRepName')(co),signBg,'374151',false),
  ...
  _w('closeModal')('excel-modal');
  _w('toast')(`📥 ${fname} 다운로드 완료`,'success');
}
...
// ==============================================================================
window._isPIMonthFullyPaid = _isPIMonthFullyPaid;
window._syncXlFields = _syncXlFields;
window.downloadPrevMonthExcel = downloadPrevMonthExcel;
window._showPIDownloadFallbackConfirm = _showPIDownloadFallbackConfirm;
window._probAutoCreateAndSave = _probAutoCreateAndSave;
window.goPayrollInputNew = goPayrollInputNew;
window.editPayroll = editPayroll;
window._fillPayrollFields = _fillPayrollFields;
window.restoreEditPayroll = restoreEditPayroll;
window.cancelEditPayroll = cancelEditPayroll;
window.handleExcelUpload = handleExcelUpload;
window.validateAndParseExcel = validateAndParseExcel;
window.showUploadReport = showUploadReport;
window.confirmBulkUpload = confirmBulkUpload;
window.openExcelModal = openExcelModal;
window.onXlCompanyChange = onXlCompanyChange;
window.downloadPayrollExcel = downloadPayrollExcel;