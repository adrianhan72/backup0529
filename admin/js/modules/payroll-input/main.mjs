...
export function loadPITargetList(){
  ...
  if(!yr || !mo){ _w('toast')('년도와 월을 선택하세요.', 'warning'); return; }

  // 등기임원 / 특수관계인 데이터 보장 로드 후 재렌더링
  const _needReload = (!window.allExecutives || window.allExecutives.length === 0) || (!window.allRelatedParties || window.allRelatedParties.length === 0);
  if(_needReload){
    Promise.all([
      window.allExecutives && window.allExecutives.length > 0 ? Promise.resolve() : loadExecutives().catch(()=>{}),
      window.allRelatedParties && window.allRelatedParties.length > 0 ? Promise.resolve() : loadRelatedParties().catch(()=>{})
  ...
  const targetContracts = getContracts().filter(c => {
  ...
  const coData = (getCompanies()||[]).find(c => c.id === coId);
  ...
  // 등기임원
  (window.allExecutives||[]).filter(e => e.company_id === coId).forEach(e => {
    targets.push({
  ...
  // 특수관계인
  (window.allRelatedParties||[]).filter(r => r.company_id === coId).forEach(r => {
    targets.push({
  ...
  if(!targets.length){
    ...
    tbody.innerHTML = targets.map(({emp, contract, _type}) => {
      ...
      if(isVirtual && emp.name){
        const matchedEmp = getEmployees().find(e => e.company_id === coId && e.name === emp.name);
        if(matchedEmp){
          const matchedContract = getContracts().find(c => 
...
/** 대상자 목록 닫기 */
export function hidePITargetList(){
 ...
 */
export function selectPITarget(empId, contractId, draftId=null){
  const emp      = getEmployees().find(e => e.id === empId);
  const contract = getContracts().find(c => c.id === contractId);
  if(!emp || !contract){ _w('toast')('직원 또는 계약 정보를 찾을 수 없습니다.', 'error'); return; }
  ...
  if(draftId){
    ...
    const existingPayroll = (getPayrolls()||[]).find(p =>
      p.employee_id===empId && p.company_id===window.currentGlobalCompanyId &&
...
export function backToPITargetList(){
...
export async function loadPIEmployees(){
  const co=window.currentGlobalCompanyId || document.getElementById('pi-company').value;
  const s=document.getElementById('pi-employee');
  s.innerHTML='<option value="">선택</option>';

  // 등기임원 / 특수관계인 데이터 보장 로드
  if(!window.allExecutives || window.allExecutives.length === 0){ await loadExecutives().catch(()=>{}); }
  if(!window.allRelatedParties || window.allRelatedParties.length === 0){ await loadRelatedParties().catch(()=>{}); }
  ...
  const coData = (getCompanies()||[]).find(c => c.id === co);
  if(coData){
    let reps = [];
    try { reps = typeof coData.representatives === 'string' ? JSON.parse(coData.representatives) : (coData.representatives || []); } catch(e){ reps = []; }
    if(!Array.isArray(reps)) reps = [];
  ...
  // 등기임원
  (window.allExecutives||[]).filter(e=>e.company_id===co).sort((a,b)=>(a.name||'').localeCompare(b.name||'','ko')).forEach(e=>{s.innerHTML+=`<option value="${e.id}" data-type="executive" style="color:#4f46e5;">${e.name} (${e.position||'등기임원'}) — 등기임원</option>`;});
...
export function calcAnnualLeaveTable(){
  ...
  const empData = (getEmployees() || []).find(e => e.id === empId) || {};
  ...
  // ── 누적 사용연차: 관리대장(window.allLeaveLedgers) 우선, 없으면 payroll 합산 ──
  //
  // [관리대장 있는 경우]
  ...
  //
  // [관리대장 없는 경우]
  //   - 기존 로직: getPayrolls()에서 이번달 편집 레코드 제외한 합산 + thisUsed
  let prevCum = 0;
  const _ledger = (window.allLeaveLedgers || []).find(r =>
 ...
 */
export function _calcAnnualAutoPayAmount(remainDays){
 ...
 */
export function onAnnualAutoChkChange(){
...
export function loadPIContract(){
  ...
  {
    const _piCandidates = getContracts().filter(c=>
      c.employee_id===empId &&
  ...
  const _piLcCoId = window.currentGlobalCompanyId || document.getElementById('pi-company')?.value;
  ...
  const _piLcCo = (typeof getCompanySnapshotAt === 'function' && _piLcContractTs)
    ? (getCompanySnapshotAt(_piLcCoId, _piLcContractTs) || getCompanies().find(c=>c.id===_piLcCoId))
    : getCompanies().find(c=>c.id===_piLcCoId);
  ...
  if(piContract){
    ...
    // ── 이번달 사용연차 초기값: 관리대장에 해당 월 데이터가 있으면 우선 적용 + max 설정 ──
    {
      ...
      if(_alInitEl){
        ...
        let _alInitVal = 0;
        if(_alYr && _alMo){
          const _alLedger = (window.allLeaveLedgers || []).find(r =>
      ...
      if(_ppEl){
        ...
        if(_isPartialTarget && _endRaw && _ppYr && _ppMo){
          ...
          if(_endDate < _monthEnd){
            ...
            const _coId  = window.currentGlobalCompanyId || document.getElementById('pi-company')?.value;
            const _co    = getCompanies().find(c => c.id === _coId);
    ...
    // ── 계약정보 카드 본문 빌드 ──────────────────────────────────────────────
    {
      ...
      let _hireDateLine = '';
      if(_isRegular){
        const _piEmpData = getEmployees().find(e => e.id === empId);
...
export function _loadPrevMonthMemos(){
  ...
  const prevPay = (getPayrolls() || []).find(p =>
    p.employee_id === empId &&
    p.pay_year    === prevYr &&
    p.pay_month   === prevMo &&
    !p.is_draft
...
export function _hidePrevMemoBanner(){
  const banner = document.getElementById('pi-prev-memo-banner');
  if(banner) banner.style.display = 'none';
}

export function _dismissPrevMemoBanner(){
  _hidePrevMemoBanner();
}
const gv=id=>{const el=document.getElementById(id);return el?parseFloat((el.value||'').replace(/,/g,''))||0:0;};
// ── 현재 선택된 고객사의 4대보험 적용 기준 반환 ──
export function _getPIInsuranceBasis(){
  // window.currentGlobalCompanyId 우선, 없으면 숨김 select value 폴백
  const coId = window.currentGlobalCompanyId || document.getElementById('pi-company')?.value;
  const co = getCompanies().find(c=>c.id===coId);
...
export function _checkPIStandardsReady(yr, mo, companyId){
  // 확정액 기준 고객사는 요율 불필요 → 항상 통과
  const _basisCoId = companyId || window.currentGlobalCompanyId || document.getElementById('pi-company')?.value;
  const _basisCo = getCompanies().find(c=>c.id===_basisCoId);
  ...
  const missing = types
    .filter(t => !window._allInsuranceRates.find(r =>
...
// ──────────────────────────────────────────────────────────────────────────────
export function _calcPIDefaultWorkDays(contract, year, month){
...
// ──────────────────────────────────────────────────────────────────────────────
export function _countWorkDays(startDate, endDate, daysPerWeek){
...
// ──────────────────────────────────────────────────────────────────────────────
export function _applyPIDefaultWorkDays(forceOverwrite){
...
//   - piContract 없으면 0 반환
// ──────────────────────────────────────────────────────────────────────────────
export function _getPIFullMonthWorkDays(){
  if(!piContract) return 0;
  const yr = parseInt(document.getElementById('pi-year')?.value)  || 0;
  const mo = parseInt(document.getElementById('pi-month')?.value) || 0;
  if(!yr || !mo) return 0;
...
export function _updatePIWorkDaysAutoLabel(result){
...
// ──────────────────────────────────────────────────────────────────────────────
export function _applyPIPayDate(forceOverwrite){
...
//   - 수습 계약이 아니면 null 반환
// ──────────────────────────────────────────────────────────────────────────────
export function _calcProbationEndDate(ct){
  if(!ct) return null;
...
// ──────────────────────────────────────────────────────────────────────────────
export function _checkPIProbationOverrun(){
...
export function onPIYearMonthChange(){
  ...
  const stdCheck = _checkPIStandardsReady(yr, mo, window.currentGlobalCompanyId);
  if(!stdCheck.ok){
    _showPIStandardsWarn(yr, mo, stdCheck.missing);
    // 입력 폼 잠금 (저장 버튼 비활성)
    _setPIInputLocked(true);
  ...
  // ── 연월 변경 시 이번달 사용연차: 관리대장에 해당 월 데이터 있으면 우선 적용 + max 설정 ──
  {
    ...
    if(_alYmEl && _empIdYm && yr && mo){
      ...
      const _alLedgerYm = (window.allLeaveLedgers || []).find(r =>
...
// ──────────────────────────────────────────────────────────────────────────────
export function _onProbSplitInputChange(){
  ...
  if(wdProbEl && maxProbDays !== Infinity){
    ...
    if(v > maxProbDays){
      ...
      if(typeof toast === 'function')
        _w('toast')(`수습 기간 근로일수는 ${maxProbDays}일을 초과할 수 없습니다.`, 'warning');
    }
  }
  if(wdPostEl && maxPostDays !== Infinity){
    ...
    if(v > maxPostDays){
      ...
      if(typeof toast === 'function')
        _w('toast')(`채용확정 기간 근로일수는 ${maxPostDays}일을 초과할 수 없습니다.`, 'warning');
    }
  }
...
// ── 급여 입력 폼 잠금/해제 ──
export function _setPIInputLocked(locked){
...
// ── 산정기준 경고 모달 표시 ──
export function _showPIStandardsWarn(yr, mo, missing){
  const moStr = String(mo).padStart(2,'0');
  ...
  _w('openModal')('pi-standards-warn-modal');
}

// ── 현재 급여 지급 년월 기준 적용 요율 조회 ──
export function _getPIRates(){
  ...
  const payDate = `${yr}-${String(mo).padStart(2,'0')}-01`;
  const find = (type) => {
    const r = window._allInsuranceRates.find(r=>
...
export function _switchInsuranceModeUI(){
...
// ──────────────────────────────────────────────────────────────────────────────
export function calcPITotalHours(){
  const thEl = document.getElementById('pi-total-hours');
  if(!thEl) return;
...
export function clampPITotalHours(){ calcPITotalHours(); calcPI(); }

// ── 근로 실적 자동 산출 ──
// 근로일수 / OT·야간·휴일 시간 입력 시 계약서 기반 금액 자동 산출 후 표시
// 실제 기본급·수당 입력 필드는 직접 건드리지 않음 (사용자 수정 우선)
export function calcPIWorkActual(){
...
export function calcPI(){
  ...
  // ── 5인 미만 사업장 판정 ─────────────────────────────────────────────────
  const _coId = window.currentGlobalCompanyId;
...
// ── 요율 기준 자동 계산 ──
export function calcPIDeductions(gross){
...
export function calcPIFixed(gross){
  if(gross===undefined){
 ...
 */
export function _getPISmallFirmInfo(coId, yr, mo){
  ...
  const execNames = (window.allExecutives||[]).filter(e => e.company_id === coId).map(e => e.name).filter(Boolean);
  const relNames  = (window.allRelatedParties||[]).filter(r => r.company_id === coId).map(r => r.name).filter(Boolean);
  const excludedNames = new Set([...repNames, ...execNames, ...relNames]);
  const repEmpIds = new Set(
    (getEmployees()||[]).filter(e => e.company_id === coId && (e.is_representative || excludedNames.has(e.name))).map(e => e.id)
  );
  const empContractMap = new Map();
  (getContracts()||[])
    .filter(c =>
...
/** 간편 래퍼: true = 5인 미만 */
export function _getPISmallFirm(coId, yr, mo){
 ...
 * @param {number} mo      급여 월
 */
export function _updatePISmallFirmBadge(sfInfo, yr, mo){
...
export function setPITransportType(type){
  // 선택옵션 제거 — 항상 self_driving 고정, no-op
}

/** JS field명(언더스코어) → HTML id용 하이픈 변환 헬퍼 */
export function _piFieldToHtmlId(field){ return field.replace(/_/g, '-'); }

export function setPIPayType(field, type){
...
export function _getPIPayTypeVal(field){
  // 'fixed'=통상임금 포함, 'daily'=제외, ''=미선택(저장 불가)
  return _piPayTypes[field] ?? '';
}

export function _resetPIPayTypes(){
 ...
 */
export function _renderPIIrregularRows(){
 ...
 *
 * @param {object|null} cfg  getCompanies()[*].allowance_config
 */
export function applyPIAllowanceConfig(cfg){
 ...
 */
export function _forceShowNonZeroPIRows(p){
 ...
 */
export function _hideZeroContractPIRows(){
  if(!piContract) return;
...
export function _getPITransportAmount(){
  return gv('pi-transport');
}
// ── 차량교통비를 각 DB 필드에 매핑해 {field: value} 반환 ──
export function _getPITransportFields(amount){
...
export function _setPIContractReadonly(on){
  // 입력 필드 잠금/해제
...
//   - 메모(note)
// ────────────────────────────────────────────────────────────────────────────
export function _resetPIInputsOnly(){
  // 1) 비정기 지급항목 금액 초기화
  [
...
export function _readPIFormSnapshot(){
  const obj = {};
...
/** 원상복구 버튼 활성/비활성 갱신 */
export function _checkPIRestoreBtn(){
 ...
 */
export function _updatePIBottomBtns(){
  // ── 취소 버튼 ──
 ...
 */
export function _showPIDraftEditModeAlert(){
 ...
 * - 신규 모드: enabled
 */
export function _updatePIDraftBtnForMode(){
 ...
 */
export function _showPIDraftSavedModal(yr, mo, timeStr){
  // 기존 모달 제거
 ...
 */
export function _showPISavedModal(empName, yr, mo, payrollId, isEdit){
...
window.backToPITargetList = backToPITargetList;
window.loadPIEmployees = loadPIEmployees;
window.calcAnnualLeaveTable = calcAnnualLeaveTable;
window._calcAnnualAutoPayAmount = _calcAnnualAutoPayAmount;
window.onAnnualAutoChkChange = onAnnualAutoChkChange;
window.loadPIContract = loadPIContract;
window._loadPrevMonthMemos = _loadPrevMonthMemos;
window._hidePrevMemoBanner = _hidePrevMemoBanner;
window._dismissPrevMemoBanner = _dismissPrevMemoBanner;
window._getPIInsuranceBasis = _getPIInsuranceBasis;
window._checkPIStandardsReady = _checkPIStandardsReady;
window._calcPIDefaultWorkDays = _calcPIDefaultWorkDays;
window._countWorkDays = _countWorkDays;
window._applyPIDefaultWorkDays = _applyPIDefaultWorkDays;
window._getPIFullMonthWorkDays = _getPIFullMonthWorkDays;
window._updatePIWorkDaysAutoLabel = _updatePIWorkDaysAutoLabel;
window._applyPIPayDate = _applyPIPayDate;
window._calcProbationEndDate = _calcProbationEndDate;
window._checkPIProbationOverrun = _checkPIProbationOverrun;
window.onPIYearMonthChange = onPIYearMonthChange;
window._onProbSplitInputChange = _onProbSplitInputChange;
window._setPIInputLocked = _setPIInputLocked;
window._showPIStandardsWarn = _showPIStandardsWarn;
window._getPIRates = _getPIRates;
window._switchInsuranceModeUI = _switchInsuranceModeUI;
window.calcPITotalHours = calcPITotalHours;
window.clampPITotalHours = clampPITotalHours;
window.calcPIWorkActual = calcPIWorkActual;
window.calcPI = calcPI;
window.calcPIManual = calcPIManual;
window.calcPIDeductions = calcPIDeductions;
window.calcPIFixed = calcPIFixed;
window._piFinalize = _piFinalize;
window._getPISmallFirmInfo = _getPISmallFirmInfo;
window._getPISmallFirm = _getPISmallFirm;
window._updatePISmallFirmBadge = _updatePISmallFirmBadge;
window.calcWeeklyHolidayPay = calcWeeklyHolidayPay;
window.setPITransportType = setPITransportType;
window._piFieldToHtmlId = _piFieldToHtmlId;
window.setPIPayType = setPIPayType;
window._getPIPayTypeVal = _getPIPayTypeVal;
window._resetPIPayTypes = _resetPIPayTypes;
window._renderPIIrregularRows = _renderPIIrregularRows;
window.applyPIAllowanceConfig = applyPIAllowanceConfig;
window._forceShowNonZeroPIRows = _forceShowNonZeroPIRows;
window._hideZeroContractPIRows = _hideZeroContractPIRows;
window._getPITransportAmount = _getPITransportAmount;
window._getPITransportFields = _getPITransportFields;
window._setPIContractReadonly = _setPIContractReadonly;
window.clearPIFields = clearPIFields;
window.clearPI = clearPI;
window._resetPIInputsOnly = _resetPIInputsOnly;
window._readPIFormSnapshot = _readPIFormSnapshot;
window._checkPIRestoreBtn = _checkPIRestoreBtn;
window._updatePIBottomBtns = _updatePIBottomBtns;
window._showPIDraftEditModeAlert = _showPIDraftEditModeAlert;
window._updatePIDraftBtnForMode = _updatePIDraftBtnForMode;
window._showPIDraftSavedModal = _showPIDraftSavedModal;
window._showPISavedModal = _showPISavedModal;