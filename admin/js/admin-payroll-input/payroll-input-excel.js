      ...
      if(_isPartialTargetEdit && _endRawEdit){
        ...
        if(_endDateEdit < _monthEndEdit){
          ...
          const _isJeonwolEdit = _coMoEdit
            ? (_coMoEdit === PAY_PERIOD_MONTH.PREV_MONTH || _coMoEdit === PAY_PERIOD_MONTH.PREV_MONTH)
            : (_coEdit?.pay_period || '').replace(/\s/g,'').startsWith(PAY_PERIOD_MONTH.PREV_MONTH) || (_coEdit?.pay_period_month === PAY_PERIOD_MONTH.PREV_MONTH);
          let _sStrEdit;
          if(_isJeonwolEdit){
...
function _fillPayrollFields(p, cfgCo){
  ...
  if(_editCo?.insurance_basis === INSURANCE_BASIS.FIXED_AMOUNT || _editCo?.insurance_basis === INSURANCE_BASIS.FIXED_AMOUNT || basis === INSURANCE_BASIS.FIXED_AMOUNT){
...
function validateAndParseExcel(wb, fileName){
  ...
  (function _updateCalcDesc(){
    ...
    if(!el) return;
    const basis = normalizeInsuranceBasis(co?.insurance_basis) || INSURANCE_BASIS.RATE_BASED;
    const isFixed = basis === INSURANCE_BASIS.FIXED_AMOUNT || basis === INSURANCE_BASIS.FIXED_AMOUNT;
    if(isFixed){
      el.innerHTML =
  ...
  if(isCardFormat){
    ...
    const coInsuranceBasis = normalizeInsuranceBasis(co?.insurance_basis) || INSURANCE_BASIS.RATE_BASED;
    const isFixedInsurance = coInsuranceBasis === INSURANCE_BASIS.FIXED_AMOUNT;
