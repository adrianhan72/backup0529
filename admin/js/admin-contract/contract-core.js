 ...
 * 선택된 고객사에 대한 아코디언 카드 렌더링 (총 7종)
 * 순서: ①계약예정 ②임시저장 ③근로계약서 미발송 ④정보제공동의서 미발송
 *        ⑤계약만료 통지대상 ⑥정규직 전환 의무 대상 ⑦수습근로자 관리
 */
  ...
  function renderOuterCard(key, icon, iconColor, title, count, desc, cols, rowsHtml, extraClass){
    ...
    div.innerHTML = `
      <div class="cont-alert-card-head" onclick="_toggleContCoCard(this)" style="cursor:pointer;">
        ...
        <div style="display:flex;align-items:center;gap:10px;">
          <span style="font-size:11.5px;font-weight:500;opacity:.7;">${desc}</span>
          <button class="cont-alert-card-chevron${isOpen?' open':''}" onclick="event.stopPropagation();_toggleContCoCard(this.closest('.cont-alert-card-head'))" title="펼치기/접기"><i class="fas fa-chevron-down"></i></button>
        </div>
      </div>
    ...
    return `<tr>
      ...
      <td style="white-space:nowrap;">
        ...
        ${docsIncomplete && !(c.status===CONTRACT_STATUS.VOIDED||c.is_voided_by_amend)
          ? `<button class="btn btn-sm btn-success" onclick="openDocsUploadModal('${c.id}')"><i class="fas fa-upload"></i> 서류 업로드</button>`
          : ''
        }
