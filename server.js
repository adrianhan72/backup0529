...
// contracts.retention_cleared 컬럼 추가 (5년 보존기간 만료 플래그)
try { db.run(`ALTER TABLE contracts ADD COLUMN retention_cleared INTEGER DEFAULT 0`); } catch(e) { /* 이미 존재함 */ }
try { db.run(`ALTER TABLE contracts ADD COLUMN retention_cleared_at TEXT`); } catch(e) { /* 이미 존재함 */ }
...
// ── 5년 보존기간 경과 계약 자동 정리 (매일 오전 4:00 KST) ──
cron.schedule('0 4 * * *', async () => {
  ...
  try {
    ...
    // 5년 이상 경과한 종료 계약 조회 (해지·만료·파기·취소)
    const oldContracts = db.all(`
      ...
      WHERE c.status IN ('terminated', 'expired', 'voided', 'canceled')
        AND (
          -- 종료 기준일: terminate_date > contract_end > voided_at
    ...
    for (const c of oldContracts) {
      try {
        ...
        console.log(`[CRON]   정리 완료: ${c.emp_name||'?'} / ${c.company_name||'?'} (종료: ${c.terminate_date||c.contract_end||'?'}, 계약ID: ${c.id})`);
      } catch (e) {
