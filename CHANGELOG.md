...
## 📋 v2.36.0 기능개선 — 퇴직금 산정 고도화 + 발송관리/연차 3탭 통합 + 고객사 우선선택 UX (2026-09-01 ~ 09-08)

### 1. 퇴직금 산정 고도화 (퇴직금산정 페이지)
- **법정 산식 적용**: `[최근 3개월 임금 합계(기본급+주휴+고정수당 전액+정기상여금+연장·야간·휴일수당) + (퇴사 전 1년 비정기상여금 × 3/12) + (연차미사용수당 × 3/12)] ÷ 3개월 달력일수(89~92일) × 30 × 재직일수 ÷ 365` (원 단위 절사)
- **최근 3개월(해지월 직전 3개월) 급여 전월 필수**: 1개월이라도 미입력 시 붉은 글씨 `산출불가(급여미입력)` + 툴팁에 미입력 월 목록 표시
- **산출 내역 레이어 팝업**: 파란 금액 클릭 시 산식·항목별 반영 금액·월별 고정합계·1일 평균임금·퇴직급여 상세 표시
- 퇴직금 산정 테이블에 **퇴직급여 열** 추가 (재직기간 다음)

### 2. 퇴직금명세서 발송 기능 신설
- 명세서 업로드/미리보기/삭제 → 알림톡·이메일·수동교부 발송
- `severance_dispatch` 테이블 신설 + 발송 이력·메시지 규칙(`severance_dispatched`, `severance_dispatch_kakao/email`)·인앱 알림 연동

### 3. 발송관리 3페이지 UX 통일 (고객사 우선선택)
- 급여명세서/근로계약서/정보제공동의서 발송 페이지에 고객사 선택 카드 → 콘텐츠 구조 적용, 테이블의 고객사 열 제거
- 페이지 간 고객사 자동선택(`currentGlobalCompanyId`) 연동
- 정보제공동의서: 탭바 우측 '동의서양식 PDF 보기' + 양식 PDF 최신본으로 교체(보기·발송 공용)

### 4. 연차관리대장 3탭 통합
- 연차관리대장에 사용촉진 발송 통합(대장/촉진발송/메시지예시), 독립 연차사용촉진 페이지·메뉴 삭제
- 촉진 발송 시 메시지 규칙(근로자/고객사) 적용 + 인앱 알림 해제 시 고객사 섹션 숨김
- 기준년도 필터바 분리, 파란 배너 제거

### 5. 퇴직금산정 페이지 3탭 구조
- 미발송 명세서/발송 이력/메시지 예시 탭 + `std-tab-panel` 공통 스타일 적용

### 6. UX/UI 개선
- 근무시간표(근로계약 입력/수정): 휴게 라벨 보라→회색, 휴게 앞 20px 여백, 근무 라벨 볼드
- 사이드바 메뉴: 폰트 13→12px, 세로 패딩 10→7px → FHD 전체화면에서 스크롤 제거
- 섹션 타이틀 여백·타이틀 폰트·헤더바 아이콘 통일, 근태 수습 필터·고객사 유지 버그 수정

### 7. 데이터 정합성 (9/1~9/3)
- 관리자/고객사 앱 JWT 인증 연동 수정 (DEV=false /tables 쓰기 401 해소)
- 근로계약 식대·차량 기본값 제거, 고정수당·월약정 데이터 보정, 스키마 현행화
- 수습 직접액: 월약정 대비 비율로 전 지급항목 재계산 + 시급 필수 검증
- 임금조건 고정 기준 도입, 계약직 연봉 필드 제거, 대시보드 배너 개편

### 8. 갱신예정 잔재 계약 처리 (9/6)
- 고아 `renewal_pending` 계약 제외: 급여입력 지급대상(시작일 경과 시)·근로계약 목록 예정사항 카드
- 예정사항 카드의 갱신예정 대상일 계산 버그 수정 (DB 미존재 컬럼 참조 제거)
- 한솔라 잔재 갱신예정 계약 파기 처리

### 9. 급여 통계 조회 — 수습 카드 조건 표시
- 시스템 설정 수습근로자 관리 **OFF** 시 직원 현황의 정규직(수습)·계약직(수습) 카드 숨김 + 3열 그리드 전환 (ON 시 5열 복원)
...
### 8. 고객사 앱 인앱 알림 표시 정리 (client)
- **검수/날인 요청 알림의 계약 정보 박스 숨김**: 알림 상세에서 `근로자명 + 계약종료일(D-day)` 박스(`notif-detail-infobox`)가 본문 규칙과 무관하게 표시되던 것 제거 — `contract_review_request`/`contract_seal_request` 타입은 박스 숨김
- **본문 파일주소 다운로드 링크**: 알림 목록·상세 본문의 http(s) URL을 클릭 시 바로 다운로드되는 링크(`notif-link`)로 변환 (`_notifBodyHtml`) — 이스케이프 후 URL만 링크 처리(문장부호 분리), XSS 방지

### 9. 고객사 앱 급여현황 — 수습 카드 숨김 (기능 OFF 시)
- 시스템 설정 `probation_feature_enabled`(수습근로자 관리)가 **OFF**일 때 고객사앱 급여현황 '직원 현황(고용형태별)'의 **정규(수습)·계약(수습) 카드 숨김**
- 클라이언트가 `system_settings`에서 수습 기능 플래그를 로드(`window._probationFeatureEnabled`)하고 `renderStats()`에서 해당 카드 `display:none` 처리 (정규직/계약직/일용직 카드는 유지)

### 10. 고객사 앱 직원 현황 — 대표자/등기임원/특수관계인 카드 + 목록 페이지
- **역할 카드 3종 추가** (정규직/계약직/일용직 앞에 배치): `대표자` / `등기임원` / `특수관계인`, 맨 뒤에 `기타` 카드 → **전 인원이 카드에 모두 표시** (기존 고용형태 NULL 직원·대표자 등 누락 문제 해결)
- **역할 판별** (`_clientEmpRole`/`_clientEmpCategory`): 대표자=`companies.representatives`, 등기임원=`registered_executives`, 특수관계인=`related_party_workers` — 클라이언트가 해당 테이블 추가 로드 (`client-core.js`)
- **목록 페이지**: `showContractsByCategory`가 역할 카테고리(대표자/등기임원/특수관계인/기타)도 처리 — `page-contracts` 목록에 직원 카드(계약 없음 포함) 렌더링, `CAT_CONFIG`에 역할별 배경/아이콘 추가
- 검증(한울테크): 대표자 1(장현우) + 정규 10 + 계약 3 + 일용 2 + 수습 2(김수민·박소연) = 합계 17 → **수습 기능 OFF 시 표시 대상 15** (아래 11번에서 수정)

- 검증(한울테크): 대표자 1(장현우) + 정규 10 + 계약 3 + 일용 2 + 수습 2(김수민·박소연) = 합계 17 → **수습 기능 OFF 시 표시 대상 15** (아래 11번에서 수정)

### 11. 고객사 앱 수습 직원 필터링 + 만료 계약 퇴직 처리 (재직 12명 정합)
- **수습 기능 OFF 시 수습 직원 전체 제외**: 고용형태가 수습(`regular_probation`/`fixed_probation`)이거나, 보유 계약 중 수습 계약이 있는 직원을 카운트·목록에서 제외
  - `_isClientProbationEmp(emp)`: 고용형태 확인 + 보유 계약(contract_type) 기준 수습 판별
  - `_clientVisibleEmps()`: `probation_feature_enabled`가 ON이면 전체, OFF면 수습 직원 필터링
  - `renderStats()`: 전체/재직/퇴직·카테고리·역할 카운트와 `showContractsByCategory()` 목록에 적용
- **만료 계약 3건 퇴직 처리** (`scripts/_tmp_mark_resigned.js`): 한울테크(comp01) 류가은(2025-06-30)·윤지현(2024-12-31)·최병준(2026-01-05) → `status='resigned'`, `resign_date=계약 종료일`
- 검증(한울테크, 수습 OFF): 전체 **15** / 재직 **12**(유효계약 11 + 대표자 1) / 퇴직 **3** · 유효계약 11건 · 카드: 대표자 1 · 정규 10(박소연 제외) · 계약 3(전원 퇴직) · 일용 1(이덕수, 김수민 제외) · 수습 카드 숨김 확인
