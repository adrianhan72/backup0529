...
> 구현 파일: `admin/js/admin-contract.js` → `calcContractSalary()`  
> 구현 파일: `admin/js/admin-payroll-input.js` → `calcPI()` / `calcPIDeductions()` / `calcPIFixed()`

### 1. 근로계약 입력 시 자동계산 (`calcContractSalary`)

#### 전제: 통상시급(hw) 직접 입력 → 나머지 역산

#### 수당 분류

| 분류 | 포함 항목 | 변수 |
|------|-----------|------|
| **통상임금 수당합** | 직책수당 + 정기상여금 + 현장수당 + 기술수당 + 면허수당 + 벽지수당 + custom(통상임금 포함) | `stdAllowSum` |
| **고정OT 수당합** | 고정연장수당 + 고정야간수당 + 고정휴일수당 | `fixedOtSum` |
| **평균임금 수당합** | 차량(fixed)+식대(fixed)+출산보육(fixed)+연구(fixed)+통신(fixed)+체력(fixed)+자기계발(fixed)+도서(fixed)+해외(fixed)+custom(통상임금 제외) | `avgAllowSum` |
| **고정급 합계** | stdAllowSum + fixedOtSum + avgAllowSum | `fixedSum` |

#### 고정OT 수당 계산

```
고정연장수당  = hw × 고정연장시간(h) × 1.5
고정야간수당  = hw × 고정야간시간(h) × 0.5
고정휴일수당  = hw × 고정휴일시간(h) × 1.5
```

#### 기본급 역산 (통상시급 입력 시)

```
월간총근로시간 = stdMonthlyH(≒209h) + 고정연장h×1.5 + 고정야간h×0.5 + 고정휴일h×1.5

기본급 = stdMonthlyH × hw − (fixedOtSum + avgAllowSum)
       = 209h × hw − (고정OT수당합 + 평균임금수당합)
```

> ⚠️ **핵심 불변식**: 통상시급이 고정된 상태에서 고정OT 시간이 늘어나면  
> fixedOtSum 증가 → **기본급이 그만큼 감소**해야 월약정임금이 유지된다.

#### 주휴수당 및 월약정임금

```
주휴수당      = 기본급 ÷ 주 소정근로일수(dpw, 전일제=5)
월약정임금    = 기본급 + 주휴수당 + 고정급
연봉(정규직)  = 월약정임금 × 12  (통상시급 입력 모드일 때만 자동갱신)
```

#### 단시간 근로 기준 월 소정근로시간

```
stdMonthlyH = (hpd × dpw + hpd) × (365/7) / 12
            ≈ hpd × (dpw + 1) × 4.345
전일제 기준(8h×5일): 8 × 6 × 4.345 ≈ 208.6 → 반올림 209h
```

---

### 2. oninput 이벤트 → 재계산 체인

| 입력 필드 | 호출 함수 | 연쇄 갱신 항목 |
|-----------|-----------|----------------|
| 통상시급 (`ct-hourly-wage`) | `calcContractSalary()` | 기본급·고정OT수당·주휴수당·월약정·연봉 |
| **고정연장시간** (`ct-fixed-ot-hours`) | `_calcFixedOtFromHours()` → `calcContractSalary()` | 고정연장수당·**기본급**·주휴수당·월약정·연봉 |
| **고정야간시간** (`ct-fixed-night-hours`) | `_calcFixedNightFromHours()` → `calcContractSalary()` | 고정야간수당·**기본급**·주휴수당·월약정·연봉 |
| **고정휴일시간** (`ct-fixed-hol-hours`) | `_calcFixedHolFromHours()` → `calcContractSalary()` | 고정휴일수당·**기본급**·주휴수당·월약정·연봉 |
| 기본급·각 수당 필드 | `calcContractSalary()` | 주휴수당·월약정·연봉 |

---

### 3. 급여 입력 시 공제 자동계산 (`calcPIDeductions`)

#### 보수월액(std) = 통상임금

```
std = 기본급 + 주휴수당 + 직책수당 + 현장수당 + 벽지수당 + 기술수당 + 면허수당 + 정기상여금 + custom(통상임금포함)
```

#### 4대보험 (요율 기준 — `insurance_basis = '요율 기준'`)

```
국민연금    = round(min(std, 상한액) × 국민연금요율)   기본 4.5%, 상한 6,370,000원
건강보험    = round(std × 건강보험요율)                기본 3.545%
장기요양보험 = round(건강보험 × 장기요양요율)           기본 12.95% (건강보험료 대비)
고용보험    = round(std × 고용보험요율)                기본 0.9%
```

> 요율은 DB `insurance_rates` 테이블에서 연도·기간별로 로드. 미등록 연도는 위 기본값 사용.

#### 소득세 간이세액 근사 (부양가족 N인)

```
과세표준 = std − 국민연금 − 건강보험 − 장기요양 − 고용보험 − 150,000
```

| 과세표준 구간 | 세율 공식 |
|---|---|
| 106만원 이하 | 0원 |
| 106만~150만 | (과세표준 − 1,060,000) × 6% |
| 150만~300만 | 26,400 + (과세표준 − 1,500,000) × 15% |
| 300만~450만 | 251,400 + (과세표준 − 3,000,000) × 24% |
| 450만~800만 | 611,400 + (과세표준 − 4,500,000) × 35% |
| 800만 초과 | 1,836,400 + (과세표준 − 8,000,000) × 38% |

```
부양가족 공제  = max(0, (N − 1) × 15,000)
소득세(최종)   = max(0, 세율공식 − 부양가족 공제)
지방소득세     = floor(소득세 × 10% / 10) × 10   ← 10원 단위 내림
```

#### 확정액 기준 (`insurance_basis = '확정액 기준'`)
- 국민연금·건강보험·장기요양·고용보험: 직접 입력
- 소득세·지방소득세: 동일 간이세액 공식 자동계산

#### 최종 집계

```
공제합계 = 국민연금 + 건강보험 + 장기요양 + 고용보험 + 소득세 + 지방소득세
         + 연말정산 + 건보정산 + 건보소급 + 건보연말 + 장기요양연말 + 선급공제
실수령액 = 지급총액 − 공제합계
```
...
```
연장근로수당 = 5인 미만 → 0,  5인 이상 → hw × h × 1.5
야간근로수당 = 5인 미만 → 0,  5인 이상 → hw × h × 0.5
휴일근로수당 = 5인 미만 → 0,  5인 이상 → hw × min(h,8) × 1.5 + hw × max(h-8,0) × 2.0
```
