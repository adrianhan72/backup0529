...
CREATE TABLE IF NOT EXISTS companies (
  ...
  proration_method TEXT DEFAULT '30day_fixed', -- 일할계산 방식 (30일 고정/월 소정근로일수)
  draft_executives TEXT DEFAULT NULL, -- 임시저장 임원
  draft_related_parties TEXT DEFAULT NULL, -- 임시저장 특수관계 인
  contacts TEXT DEFAULT '[]', -- 연락처
  premium_mode TEXT DEFAULT NULL -- 가산수당 지급 기준 (none=5인 미만 시 가산 미적용, always=5인 미만이어도 가산 지급)
);
...
CREATE TABLE IF NOT EXISTS employees (
  ...
  career_history TEXT, -- 경력·이력
  military_status TEXT, -- 병역
  education TEXT, -- 최종학력
  major TEXT, -- 전공
  certifications TEXT, -- 자격증
  language_skills TEXT, -- 어학능력
